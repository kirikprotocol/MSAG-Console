...
#ifdef NEWLOGGER
size_t RollingIntervalAppender::logPrefix(char* buf, size_t bufsize, timeval tp, const char logLevelName, const char* category) throw()
#else
...
void RollingIntervalAppender::write(timeval tp, char* buffer, size_t length) throw()
{
  time_t dat = tp.tv_sec;
  buffer[length++] = '\n';
