...
////////////////////// constructors
Logger::Logger(const char * const logCategoryName, const LogLevel logLevel, Appender * const app) :
#ifdef NEWLOGGER
prefixLength(app->getPrefixLength()),
#endif
logLevel(logLevel), name(cStringCopy(logCategoryName)), appender(app)// , mutex(MTXFORCENOCHECK)
{
}
...
void Logger::logva_(timeval tv,const LogLevel _logLevel, const char * const stringFormat, va_list args) throw()
{
    ...
    if (isInitialized()) {
        ...
        try {
            ...
            appender->write(tv,logChars[_logLevel],this->name,buffer,bufsize);
