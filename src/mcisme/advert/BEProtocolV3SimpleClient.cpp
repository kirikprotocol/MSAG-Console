...
#include "BEProtocolV3SimpleClient.hpp"

namespace smsc {
namespace mcisme {

BEProtocolV3SimpleClient::BEProtocolV3SimpleClient(const std::string& host, int port)
  : AdvertisingImpl(host, port)
{}

void
BEProtocolV3SimpleClient::parseBannerResponse(const char* packet, uint32_t total_packet_len,
                                              ...
                                              uint32_t* rotator_id, std::string* service_name)
{
  ...
  if ( cmdType != CMD_BANNER_RSP ) {
    smsc_log_error(_logger, "BEProtocolV3SimpleClient::parseBannerResponse::: incorrect packet type %d", cmdType);
    throw ProtocolError();
  }
  ...
  while (offset < total_packet_len) {
    ...
    case CHARSET_TAG:
      ...
      break;
    case SERVICE_NAME_TAG:
       offset += extractParamValue(curPtrInBuf, total_packet_len - offset, service_name, len);
       break;
    case BANNER_ERRCODE_TAG:
      offset += extractParamValue(curPtrInBuf, total_packet_len - offset, &errCode);
      break;
    default:
      smsc_log_error(_logger, "BEProtocolV3SimpleClient::parseBannerResponse::: got unknown parameter type 0x%X", tag);
      throw ProtocolError();
    }
  }
  smsc_log_info(_logger, "got BannerResponse: transactionId=%u,bannerId=%d,ownerId=%u,rotatorId=%u,errcode=%u,service_name='%s'",
                *transaction_id, *banner_id, *owner_id, *rotator_id, errCode, service_name->c_str());
  if (errCode && *banner_id != -1) {
    smsc_log_error(_logger, "BEProtocolV3SimpleClient::parseBannerResponse::: protocol violation - wrong combination of errCode=%d and bannerId=%d",
                   errCode, *banner_id);
    throw ProtocolError();
  }
}
...
uint32_t
BEProtocolV3SimpleClient::prepareBannerReqCmd(util::SerializationBuffer* req /* �����*/,
                                              BannerRequest* par /*��������� ������� ������*/)
{
  ...
  case TRANSPORT_TYPE_MMS:
    break;
  default:
    throw util::Exception("BEProtocolV3SimpleClient::prepareBannerReqCmd::: invalid transportType=%d", par->transportType);
  }
...
uint32_t
BEProtocolV3SimpleClient::prepareErrorInfoCmd(util::SerializationBuffer* req /* �����*/,
                                              const BannerRequest& par /*��������� ������� ������*/,
                                              uint32_t err_code)
{
  std::string::size_type svcNameLen = par.serviceName.length();
  uint32_t totalPacketSize = prepareHeader(CMD_ROLLBACK_REQ, static_cast<uint32_t>(CMD_ROLLBACK_REQ_BODY_LEN + svcNameLen), req);
  ...
  req->WriteNetInt32(SERVICE_NAME_TAG);
  req->WriteNetInt32(static_cast<uint32_t>(svcNameLen));
  req->Write(par.serviceName.c_str(), svcNameLen);
