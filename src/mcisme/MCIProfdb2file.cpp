...
//        getProStmt->setString(1, abonent);
		...
		Statement* getProStmt1 = dsStatConnection->getStatement(GET_COUNT_ID, GET_COUNT_SQL);
        if (!getProStmt1)
			printf("throw Exception 1");
        
//        getProStmt->setString(1, abonent);
        std::auto_ptr<ResultSet> rsGuard1(getProStmt1->executeQuery());
        ResultSet* rs1 = rsGuard1.get();
        if (!rs1)
            printf("throw Exception 2");
        ...
        {
			printf("!!!!!!!!!\n");
            int c = rs1->isNull(1) ? 0:rs1->getUint32(1);
			printf("1 = %d\n", c);
        } 
		else
		{
