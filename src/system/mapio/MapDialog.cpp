...
USHORT_T MapDialog::Et96MapV1ForwardSmMT_MOConf (
    ET96MAP_LOCAL_SSN_T localSsn,
    ET96MAP_DIALOGUE_ID_T dialogueId,
    ET96MAP_INVOKE_ID_T invokeId,
    ET96MAP_ERROR_FORW_SM_MT_T *errorForwardSMmt_sp,
...
{
#if defined USE_MAP  
  __trace2__("MAP::MapDialog::Et96MapV1ForwardSmMT_MOConf");
  if ( sms.get() != 0 ){
  __trace2__("MAP::DELIVERY_SM %d.%d.%s -> %d.%d.%s",
             sms->getOriginatingAddress().getTypeOfNumber(),
             sms->getOriginatingAddress().getNumberingPlan(),
  ...
  if ( errorForwardSMmt_sp ){
    __trace2__("MAP::Et96MapV1ForwardSmMT_MOConf:did 0x%x errorForwardSMmt_sp->errorCode 0x%x",dialogid,errorForwardSMmt_sp->errorCode);
    bool fatal = false;
    switch( errorForwardSMmt_sp->errorCode ){
    case 27: /*Absent*/
    case 31: /*Busy*/
    ...
    MapDialogContainer::getInstance()->getProxy()->putIncomingCommand(cmd);
    __trace2__("MAP::Et96MapV1ForwardSmMT_MOConf:did 0x%x/0x%x was send %s to SMSC",dialogid,smscDialogId, 
               fatal?"CMD_ERR_FATAL":"CMD_ERR_TEMP");
  }else if (provErrCode_p){
    __trace2__("MAP::Et96MapV1ForwardSmMT_MOConf:did 0x%x/0x%x *provErrCode_p 0x%x",dialogid,smscDialogId,*provErrCode_p);
    SmscCommand cmd;
    cmd = SmscCommand::makeDeliverySmResp("0",this->smscDialogId,MAKE_ERRORCODE(CMD_ERR_TEMP,MAP_NETWORKERROR));
    MapDialogContainer::getInstance()->getProxy()->putIncomingCommand(cmd);
    __trace2__("MAP::Et96MapV1ForwardSmMT_MOConf:did 0x%x/0x%x was send CMD_ERR_TEMP to SMSC",dialogid,smscDialogId);
  }else{
    SmscCommand cmd = SmscCommand::makeDeliverySmResp("0",this->smscDialogId,MAKE_ERRORCODE(CMD_OK,0));
    MapDialogContainer::getInstance()->getProxy()->putIncomingCommand(cmd);
    __trace2__("MAP::Et96MapV1ForwardSmMT_MOConf:did 0x%x/0x%x was send OK to SMSC",dialogid,smscDialogId);
  }
#else
#endif  
  return ET96MAP_E_OK;
...
#if defined USE_MAP
  try{
    __trace2__("MAP::MapDialog::Et96MapV1ForwardSmMOInd");  
    setInvokeId(invokeId);
    SMS sms;
    Address src_addr;
    Address dest_addr;
...
USHORT_T MapDialog::Et96MapV1SendRInfoForSmConf (
    ET96MAP_LOCAL_SSN_T localSsn,
    ...
    ET96MAP_PROV_ERR_T *provErrCode_p)
{
  ...
  if ( errorSendRoutingInfoForSm_sp != 0 ){
    ...
    MapDialogContainer::getInstance()->getProxy()->putIncomingCommand(cmd);
    __trace2__("MAP::Et96MapV1SendRInfoForSmConf errorSendRoutingInfoForSm_sp->errorCode 0x%hx",
               errorSendRoutingInfoForSm_sp->errorCode);
    throw runtime_error("MAP::Et96MapV1SendRInfoForSmConf error");
  }
  if ( provErrCode_p != 0 ){
    // error hadling
    SmscCommand cmd;
    ...
    MapDialogContainer::getInstance()->getProxy()->putIncomingCommand(cmd);
    __trace2__("MAP::Et96MapV1SendRInfoForSmConf provErrCode_p 0x%hx",*provErrCode_p);
    throw runtime_error("MAP::Et96MapV1SendRInfoForSmConf error");
  }
