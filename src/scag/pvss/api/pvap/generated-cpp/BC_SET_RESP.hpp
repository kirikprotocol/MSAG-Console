        ...
        throw (PvapException)
    {
        ...
        try {
            do {
                ...
                default:
                    throw InvalidFieldTypeException(data_->isRequest(),"BC_SET_RESP", getSeqNum(),tag);
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw PvapSerializationException( data_->isRequest(),
...
protected:
    void checkFields() const throw (PvapException)
    {
        // using parent check
        if ( !data_->isValid() ) {
            throw MessageIsBrokenException(data_->isRequest(), getSeqNum(), "message BC_SET_RESP is broken: %s",data_->toString().c_str());
        }
    }
