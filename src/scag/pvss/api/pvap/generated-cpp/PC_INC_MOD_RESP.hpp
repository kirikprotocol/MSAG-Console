...
public:
    PC_INC_MOD_RESP( int seqNum ) :
    owned_(true),
    data_(new IncResponse(seqNum))
    {
    }

    PC_INC_MOD_RESP( IncResponse* other ) :
    owned_(false),
    data_(other)
    {
    }
        ...
        void serialize( const PVAP& proto, DataStream& writer ) const throw (PvapException)
    {
        ...
        try {
            ...
            throw PvapSerializationException( data_->isRequest(),
                                              data_->getSeqNum(),
                                              "writing field statusValue in PC_INC_MOD_RESP: %s",
                                              e.what() );
        }
        try {
            // printf( "write pos=%d field=%d\n", ds.getPos(), resultTag );
            ...
            throw PvapSerializationException( data_->isRequest(),
                                              data_->getSeqNum(),
                                              "writing field result in PC_INC_MOD_RESP: %s",
                                              e.what() );
        }
        // optional fields
    }
        ...
        throw (PvapException)
    {
        ...
        try {
            do {
                ...
                default:
                    throw InvalidFieldTypeException(data_->isRequest(),"invalid field in PC_INC_MOD_RESP", data_->getSeqNum(),tag);
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw PvapSerializationException( data_->isRequest(),
                                              data_->getSeqNum(),
                                              "reading field tag=%d of PC_INC_MOD_RESP: %s",
                                              tag, e.what() );
        }
        checkFields();
    }
...
protected:
    void checkFields() const throw (PvapException)
    {
        // using parent check
        if ( !data_->isValid() ) {
            throw MessageIsBrokenException(data_->isRequest(), "message PC_INC_MOD_RESP is broken: " + toString(), data_->getSeqNum() );
        }
    }

private:
    PC_INC_MOD_RESP( const PC_INC_MOD_RESP& );
    PC_INC_MOD_RESP& operator = ( const PC_INC_MOD_RESP& );
