        ...
        /* throw (PvapException) */ 
    {
        ...
        try {
            do {
                ...
                default:
                    throw InvalidFieldTypeException(data_->isRequest(),"PC_SET_RESP", data_->getSeqNum(),tag);
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw PvapSerializationException( data_->isRequest(),
...
protected:
    void checkFields() const /* throw (PvapException) */ 
    {
        // using parent check
        if ( !data_->isValid() ) {
            throw MessageIsBrokenException(data_->isRequest(), data_->getSeqNum(), "message PC_SET_RESP is broken: %s",data_->toString().c_str());
        }
    }
