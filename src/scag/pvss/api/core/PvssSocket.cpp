...
PvssSocketBase( new smsc::core::network::Socket,
                core.getConfig().getHost().c_str(),
                core.getConfig().getPort() ),
core_(core),
connectionTmo_(5),
rdBuflen_(0),
writer_(0),
...
bool PvssSocket::wantToSend( util::msectime_type currentTime )
{
    ...
    while ( ! expired.empty() ) {
        ctx = expired.front();
        expired.pop_front();
        core_.reportPacket(ctx->seqNum,*this,Core::EXPIRED);
        delete ctx;
    }
    return result;
}
...
void PvssSocket::sendData()
{
    ...
    if ( wrBuffer_.GetPos() < wrBuffer_.getSize() ) {
        ...
        res = getSocket()->Write( wrBuffer_.get()+wrBuffer_.GetPos(), res );
        if (res<=0) {
            smsc_log_warn(log_,"sendData: write failed: %d", res);
            core_.reportPacket(wrContext_->seqNum, *this, Core::FAILED);
            wrContext_.reset(0);
    ...
    core_.reportPacket(wrContext_->seqNum, *this, Core::SENT);
    wrContext_.reset(0);
}
...
void PvssSocket::processInput()
{
    if ( rdBuffer_.GetPos() < 4 ) {
        // reading length
        int res;
        res = getSocket()->Read(rdBuffer_.get()+rdBuffer_.GetPos(),int(4-rdBuffer_.GetPos()));
        if (res <= 0) {
            if ( rdBuffer_.GetPos() > 0 ) {
                core_.handleError(PvssException(PvssException::IO_ERROR,
                                                "error reading packet length res=%d, conn=%d",
                                                res,getSocket()->isConnected() ? 1 : 0),
                                  *this);
            } else {
                core_.closeChannel( *this );
            }
            return;
        }
        lastActivity_ = util::currentTimeMillis();
    ...
    // reading data
    int res;
    res = getSocket()->Read(rdBuffer_.get()+rdBuffer_.GetPos(),int(rdBuflen_-rdBuffer_.GetPos()));
    if ( res <= 0 ) {
...
void PvssSocket::connect() /* throw (exceptions::IOException) */ 
{
    ...
    if ( getSocket()->Init(host_.c_str(),port_,connectionTmo_) == -1 ) {
        smsc_log_warn(log_,"cannot init socket %p on channel %p", getSocket(), this );
        throw exceptions::IOException("cannot init socket %p on channel %p", getSocket(), this);
    }
    if ( getSocket()->Connect() == -1 ) {
        smsc_log_info(log_,"connect failed");
        throw exceptions::IOException("Connect() failed for socket %p on channel %p", getSocket(), this);
    }
}
