  ...
  for (;;) {
    ...
    for (vector<PersPacket*>::iterator i = process.begin(); i != process.end(); ++i) {
      ...
      try {
        ...
        (*i)->createResponse(RESPONSE_ERROR);
...
void AbonentStorageProcessor::process(PersPacket* packet) {
  ...
  if (!elstorage) {
    ...
    packet->createResponse(RESPONSE_ERROR);
    return;
  }
...
void InfrastructStorageProcessor::process(PersPacket* packet) {
  ...
  case PT_OPERATOR: dblog = olog_; storage = operator_; break;
  case PT_PROVIDER: dblog = plog_; storage = provider_; break;
  case PT_SERVICE:  dblog = slog_; storage = service_; break;
  default: 
    smsc_log_error(logger_, "cx %p unknown profile type %d", packet, packet->profileType);
    packet->createResponse(RESPONSE_BAD_REQUEST);
    return;
  }
