...
PersPacket* ConnectionContext::parsePacket(const time_t& now) {
  PersServerResponseType response = RESPONSE_BAD_REQUEST;
  try {
    sequenceNumber_ = 0;
    if (async_) {
    ...
    if (cmd == PC_BIND_ASYNCH) {
      smsc_log_debug(logger_, "Asynch Bind received");
      if (async_) {
        createFakeResponse(RESPONSE_ERROR);
      } else {
        createFakeResponse(RESPONSE_OK);
        async_ = true;
      }
      return NULL;
    }
    ...
    if (notSupport(cmd)) {
      createFakeResponse(RESPONSE_NOTSUPPORT);
      return NULL;
    }
