...
class BC_INC_MOD_RESP : public BC_CMD 
{
public:
    inline int getId() const throw () { return TypeId<BC_INC_MOD_RESP>::getId(); }
...
protected:
    void checkFields() const throw (MandatoryFieldMissingException)
    {
        ...
        if (!statusFlag) {
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "status", "BC_INC_MOD_RESP");
            throw MandatoryFieldMissingException(buf);
        }
        if (!intValueFlag) {
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "intValue", "BC_INC_MOD_RESP");
            throw MandatoryFieldMissingException(buf);
        }
