...
class BC_SET_RESP : public BC_CMD 
{
public:
    inline int getId() const throw () { return TypeId<BC_SET_RESP>::getId(); }
...
protected:
    void checkFields() const throw (MandatoryFieldMissingException)
    {
        ...
        if (!statusFlag) {
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "status", "BC_SET_RESP");
