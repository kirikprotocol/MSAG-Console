...
protected:
    void checkFields() const throw (PvapException)
    {
        // using parent check
        if ( !data_->isValid() ) {
            throw MessageIsBrokenException(data_->isRequest(), "message BC_INC_RESP is broken: " + toString(), data_->getSeqNum() );
        }
    }

private:
    BC_INC_RESP( const BC_INC_RESP& );
    BC_INC_RESP& operator = ( const BC_INC_RESP& );
