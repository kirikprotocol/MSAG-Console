...
protected:
    void checkFields() const throw (MandatoryFieldMissingException)
    {
        ...
        if (!profileTypeFlag) {
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "profileType", "PC_DEL");
            throw MandatoryFieldMissingException(buf);
        }
        if (!varNameFlag) {
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "varName", "PC_DEL");
            throw MandatoryFieldMissingException(buf);
        }
        ...
        if (!abonentKeyFlag
            ...
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "abonentKey", "PC_DEL");
            throw MandatoryFieldMissingException(buf);
        }
        if (!profileKeyFlag
            && (profileType>1)
            ...
            char buf[256];
            snprintf( buf, sizeof(buf), "field=%s msg=%s", "profileKey", "PC_DEL");
            throw MandatoryFieldMissingException(buf);
        }
    }
