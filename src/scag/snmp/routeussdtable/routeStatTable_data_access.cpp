 ...
 **********************************************************************/
/*
 * EYELINE-MSAG-MIB::routeStatTable is subid 31 of msag.
...
{
    DEBUGMSGTL(("verbose:routeStatTable_container_init","called\n"));
...
{
  DEBUGMSGTL(("verbose:routeStatTable_container_shutdown","called\n"));
...
int fillNextCounter(netsnmp_container* container, const char* routeId, long routeStatIndex, stat::CommonPerformanceCounter* counter)
{
  ...
  if (NULL == rec)
  {
    smsc_log_error(log, "routeUssdTable_cache_load: memory allocation failed");
    return -1;
  }

  if( MFD_SUCCESS != routeStatTable_indexes_set(rec, routeStatIndex) )
  {
    smsc_log_error(log, "routeUssdTable_cache_load: error setting index while loading routeStatTable data");
    routeStatTable_release_rowreq_ctx(rec);
    return -2;
  }

  logIndexDebug(log, "routeUssdTable", routeStatIndex, rec->tbl_idx.routeStatIndex, rec->oid_idx, rec->oid_tmp, MAX_routeStatTable_IDX_LEN);

  if ( !fillRecord(rec, sid, counter) )
  {
    smsc_log_error(log, "routeUssdTable_cache_load fillRecord error");
    return -3;
  }
  ...
  if ( 0 != rc )
    smsc_log_error(log, "routeUssdTable_cache_load CONTAINER_INSERT returns(%d)", rc);
  return rc;
}
...
int routeStatTable_cache_load(netsnmp_container* container)
{
  ...
  {
    smsc_log_error(log, "routeUssdTable_cache_load error: container is NULL");
    return MFD_RESOURCE_UNAVAILABLE;
  }
  smsc::core::buffers::Hash<stat::CommonPerformanceCounter*>& h0 = scag2::snmp::getStatMan()->getRouteCounters(1); //ussd
//  smsc_log_debug(log, "routeUssdTable_cache_load: getCounters(0) ok, %d entries", h0.GetCount());

  retCode = loadHashToContainer(container, h0, recCount);
  if ( 0 == recCount && fillEmptyData() )  // fill zero data if counters hash is empty
  {
    smsc_log_debug(log, "routeUssdTable_cache_load: no records, make fake counters");
...
//  smsc_log_debug(log, "routeUssdTable_cache_load: inserted %d records, retCode=%d", recCount, retCode);
  return retCode;
}
...
{
//  smsc_log_debug(log, "routeUssdTable_cache_free called");
