...
{
    DEBUGMSGTL(("verbose:smeSmsTable_container_init","called\n"));
    
    if((NULL == cache) || (NULL == container_ptr_ptr)) {
      snmp_log(LOG_ERR, "bad container param to smeSmsTable_container_init\n");
      return;
    }
...
{
  DEBUGMSGTL(("verbose:smeSmsTable_container_shutdown","called\n"));
  if (NULL == container_ptr) {
    snmp_log(LOG_ERR,"bad params to smeSmsTable_container_shutdown\n");
    return;
  }
...
int fillNextCounter(netsnmp_container* container, const char* sysId, stat::CommonPerformanceCounter* counter)
{
  ...
  if (sysId) {
    try {
      ...
      smsc_log_error(log, "smeSmsTable_cache_load: error, SmppManager::getSmeIndex('%s') unavailable", sysId);
      return -4;
    }
  }
  ...
  do {
    ...
    if (!rec) {
      smsc_log_error(log, "smeSmsTable_cache_load: memory allocation failed");
      result = -1;
      break;
    }

    if( MFD_SUCCESS != smeStatTable_indexes_set(rec, smeStatIndex) ) {
      smsc_log_error(log, "smeSmsTable_cache_load: error setting index while loading smeStatTable data");
      smeStatTable_release_rowreq_ctx(rec);
      result = -2;
      break;
    }

//    logIndexDebug(log, "smeSmsTable", smeStatIndex, rec->tbl_idx.smeStatIndex, rec->oid_idx, rec->oid_tmp, MAX_smeStatTable_IDX_LEN);

    if ( !fillRecord(rec, sid, counter) ) {
      smsc_log_error(log, "smeSmsTable_cache_load: fillRecord error");
      result = -3;
      break;
    }
    ...
    if ( 0 != rc ) {
      smsc_log_error(log, "smeSmsTable_cache_load: CONTAINER_INSERT(%s) returns(%d)", sysId, rc);
      result = rc;
      break;
    }
  } while (0);
...
int smeStatTable_cache_load(netsnmp_container* container)
{
  ...
  {
    smsc_log_error(log, "smeSmsTable_cache_load error: container is NULL");
    return MFD_RESOURCE_UNAVAILABLE;
  }
  smsc::core::buffers::Hash<stat::CommonPerformanceCounter*>& h0 = getStatMan()->getCounters(0, 0);

  retCode = loadHashToContainer(container, h0, recCount);
  if ( retCode == MFD_SUCCESS )
  {
    smsc::core::buffers::Hash<stat::CommonPerformanceCounter*>& h1 = getStatMan()->getCounters(1, 0);
    retCode = loadHashToContainer(container, h1, recCount);
  }
  if ( 0 == recCount && fillEmptyData() )  // fill zero data if counters hash is empty
  {
    smsc_log_debug(log, "smeSmsTable_cache_load: no records, make fake counters");
