...
void _smeStatTable_initialize_interface(smeStatTable_registration_ptr reg_ptr,  u_long flags)
{
   ...
   */
  DEBUGMSGTL(("smeStatTable:init_smeStatTable", "Registering smeUssdTable as a mibs-for-dummies table.\n"));
  handler = netsnmp_baby_steps_access_multiplexer_get(access_multiplexer);
//1
  reginfo = netsnmp_handler_registration_create("smeUssdTable", handler,
                                                smeStatTable_oid,
  ...
  snmp_log(LOG_INFO,"register table smeUssdTable, success\n");
  smsc_log_debug(log, "register table smeUssdTable, success");
...
smeStatTable_rowreq_ctx* smeStatTable_allocate_rowreq_ctx(void)
{
    ...
    if(NULL == rowreq_ctx) {
      smsc_log_error(log, "Couldn't allocate memory for a smeStatTable_rowreq_ctx");
                             ...
                             netsnmp_request_info *requests)
{
  ...
  if (MFD_SUCCESS != rc) {
       ...
       * nothing we can do about it but log it
       */
    smsc_log_error(log, "smeStatTable error %d from smeStatTable_post_request", rc);
  ...
  netsnmp_variable_list *var, int column )
{
    ...
    default:
      smsc_log_error(log, "unknown column %d in _smeStatTable_get_column", column);
