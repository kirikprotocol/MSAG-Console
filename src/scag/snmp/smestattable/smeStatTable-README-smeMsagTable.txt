...
------------------------------------------------------------------------
  ...
  the smeStatTable table.  The agent will start by looking for the right
  ...
  file defaults/table-smeStatTable.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      smeStatTable_allocate_data
      smeStatTable_release_data
    ...
    will be generated:
      ...
      smeStatTable_rowreq_ctx_init
      smeStatTable_rowreq_ctx_cleanup
 ...
 Index(es) for the smeStatTable table
  ------------------------------------------------------------
  The index(es) for the smeStatTable table are:
...
************************************************************************
smeStatTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the smeStatTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: smeStatTable_data_access.[c|h]
------------------------------------------------------------------------
  The smeStatTable_data_access file contains the interface to your data in
  ...
  FUNC : smeStatTable_indexes_set
  WHERE: smeStatTable_data_access.c
...
File: smeStatTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatSystemId_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatAccepted_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatRejected_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatDelivered_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatGwRejected_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : smeStatFailed_get
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the smeStatTable table.

To watch the flow of the smeStatTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        smeStatTable
        verbose:smeStatTable
        internal:smeStatTable

e.g.
        snmpd -f -Le -DsmeStatTable,verbose:smeStatTable,internal:smeStatTable
...
/*---------------------------------------------------------------------
 * EYELINE-MSAG-MIB::smeStatEntry.smeStatIndex
