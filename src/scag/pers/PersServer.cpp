...
void PersServer::execCommand(SerialBuffer& isb, SerialBuffer& osb)
{
    ...
    string addr_key = str_key;
    Address addr_key(str_key.c_str());
    if (str_key[0] != '.') {
      addr_key.setNumberingPlan(1);
      addr_key.setTypeOfNumber(1);
    }
    smsc_log_debug(plog, "profile key=%s", addr_key.toString().c_str());
    switch(cmd)
    {
        case PC_DEL:
            isb.ReadString(name);
            DelCmdHandler(pt, int_key, addr_key.toString(), name, osb);
            return;
        case PC_SET:
            prop.Deserialize(isb);
            SetCmdHandler(pt, int_key, addr_key.toString(), prop, osb);
            return;
        case PC_GET:
            isb.ReadString(name);
            GetCmdHandler(pt, int_key, addr_key.toString(), name, osb);
            return;
        case PC_INC:
            prop.Deserialize(isb);
            IncCmdHandler(pt, int_key, addr_key.toString(), prop, osb);
            return;
        case PC_INC_MOD:
        {
            int inc = isb.ReadInt32();
            prop.Deserialize(isb);
            IncModCmdHandler(pt, int_key, addr_key.toString(), prop, inc, osb);
			return;
        }
        case PC_BATCH:
        {
