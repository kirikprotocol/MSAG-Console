...
void CommandDispatcher::IncCmdHandler(ProfileType pt, uint32_t int_key, string& str_key, string& name, int32_t inc, SerialBuffer *sb)
{
	Property prop;
	bool exists = false;
	if(pt == PT_ABONENT)
	{
		smsc_log_debug(log, "IncCmdHandler AbonentStore: key=%s, name=%s, inc = %d", str_key.c_str(), name.c_str(), inc);
		exists = AbonentStore->incProperty(StringProfileKey(str_key.c_str()), name.c_str(), inc);
	}
	else
		for(int i = 0; i < INT_STORE_CNT; i++)
			if(pt == int_store[i].pt)
			{
				smsc_log_debug(log, "IncCmdHandler store=%d, key=%d, name=%s, inc = %d", pt, int_key, name.c_str(), inc);
				exists = int_store[i].store->incProperty(IntProfileKey(int_key), name.c_str(), inc);
			}
	if(exists)
		SendResponse(sb, RESPONSE_OK);
	else
...
void CommandDispatcher::Execute(SerialBuffer* sb)
{
	...
	try{
		...
		switch(cmd)
		{
			...
			case PC_INC:
				sb->ReadString(name);
				int32_t inc = (int32_t)sb->ReadInt32();
				IncCmdHandler(pt, int_key, str_key, name, inc, sb);
				return;
		}
	}
