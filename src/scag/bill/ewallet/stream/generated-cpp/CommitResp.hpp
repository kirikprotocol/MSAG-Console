...
#ifndef SCAG_BILL_EWALLET_STREAM_COMMITRESP_HPP
#define SCAG_BILL_EWALLET_STREAM_COMMITRESP_HPP
...
public:
    ...
    uint8_t getStatusValue() const {
        if (!statusValueFlag) {
            throw Exception( excType(), "CommitResp has no field statusValue");
        }
        return statusValue;
    }
        ...
        void serialize( const Protocol& proto, DataStream& writer ) const
    {
        ...
        try {
            ...
            throw Exception( excType(),
                             "writing field statusValue in CommitResp: %s",
                             e.what() );
        }
        // optional fields
    }
        ...
        try {
            do {
                ...
                case statusValueTag: {
                    if (statusValueFlag) {
                        throw Exception( excType(),
                                         "duplicate field statusValue of CommitResp" );
                    }
                ...
                default:
                    throw Exception( excType(),
                                     "CommitResp has invalid field:%u", tag );
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw Exception( excType(),
                             "reading field tag=%u of CommitResp: %s",
                             tag, e.what() );
        }
...
protected:
    void checkFields() const
    {
        if ( !statusValueFlag ) {
            throw Exception( excType(),
                             "message CommitResp does not have required field statusValue" );
        }
 
    }

private:
    CommitResp( const CommitResp& );
    CommitResp& operator = ( const CommitResp& );
