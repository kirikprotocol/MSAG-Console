...
#ifndef SCAG_BILL_EWALLET_STREAM_PINGRESP_HPP
#define SCAG_BILL_EWALLET_STREAM_PINGRESP_HPP
...
public:
    ...
    uint8_t getStatusValue() const {
        if (!statusValueFlag) {
            throw Exception( excType(), "PingResp has no field statusValue");
        }
        return statusValue;
    }
        ...
        void serialize( const Protocol& proto, DataStream& writer ) const
    {
        ...
        try {
            ...
            throw Exception( excType(),
                             "writing field statusValue in PingResp: %s",
                             e.what() );
        }
        // optional fields
    }
        ...
        try {
            do {
                ...
                case statusValueTag: {
                    if (statusValueFlag) {
                        throw Exception( excType(),
                                         "duplicate field statusValue of PingResp" );
                    }
                ...
                default:
                    throw Exception( excType(),
                                     "PingResp has invalid field:%u", tag );
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw Exception( excType(),
                             "reading field tag=%u of PingResp: %s",
                             tag, e.what() );
        }
...
protected:
    void checkFields() const
    {
        if ( !statusValueFlag ) {
            throw Exception( excType(),
                             "message PingResp does not have required field statusValue" );
        }
 
    }

private:
    PingResp( const PingResp& );
