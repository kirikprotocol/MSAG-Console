...
#ifndef SCAG_BILL_EWALLET_STREAM_INFORESP_HPP
#define SCAG_BILL_EWALLET_STREAM_INFORESP_HPP
...
protected:
    ...
    static const int chargeThresholdTag = 16;
...
public:
    ...
    uint8_t getStatusValue() const {
        if (!statusValueFlag) {
            throw Exception( excType(), "InfoResp has no field statusValue");
        }
        return statusValue;
    }
    ...
    int32_t getAmount() const {
        if (!amountFlag) {
            throw Exception( excType(), "InfoResp has no field amount");
        }
        return amount;
    }
    ...
    int32_t getChargeThreshold() const {
        if (!chargeThresholdFlag) {
            throw Exception( excType(), "InfoResp has no field chargeThreshold");
        }
        ...
        void serialize( const Protocol& proto, DataStream& writer ) const
    {
        ...
        try {
            ...
            throw Exception( excType(),
                             "writing field statusValue in InfoResp: %s",
                             e.what() );
        }
        // optional fields
        if ( amountFlag ) {
            ...
            try {
                ...
                throw Exception( excType(),
                                 "writing field amount in InfoResp: %s",
                                 e.what() );
            }
        }
        if ( chargeThresholdFlag ) {
            ...
            try {
                ...
                throw Exception( excType(),
                                 "writing field chargeThreshold in InfoResp: %s",
                                 e.what() );
            }
        }
    }
        ...
        try {
            do {
                ...
                case statusValueTag: {
                    if (statusValueFlag) {
                        throw Exception( excType(),
                                         "duplicate field statusValue of InfoResp" );
                    }
                ...
                case amountTag: {
                    if (amountFlag) {
                        throw Exception( excType(),
                                         "duplicate field amount of InfoResp" );
                    }
                ...
                case chargeThresholdTag: {
                    if (chargeThresholdFlag) {
                        throw Exception( excType(),
                                         "duplicate field chargeThreshold of InfoResp" );
                    }
                ...
                default:
                    throw Exception( excType(),
                                     "InfoResp has invalid field:%u", tag );
                }
            } while ( true );
        } catch ( exceptions::IOException e ) {
            throw Exception( excType(),
                             "reading field tag=%u of InfoResp: %s",
                             tag, e.what() );
        }
...
protected:
    void checkFields() const
    {
        if ( !statusValueFlag ) {
            throw Exception( excType(),
                             "message InfoResp does not have required field statusValue" );
        }
 
    }

private:
    InfoResp( const InfoResp& );
    InfoResp& operator = ( const InfoResp& );
