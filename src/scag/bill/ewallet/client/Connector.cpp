...
Connector::Connector( ClientCore& core ) :
SocketTask(core,"ewall.cnct") {}
...
void Connector::connectSocket( proto::Socket& socket )
{
...
bool Connector::setupSockets(util::msectime_type currentTime )
{
    ...
    for ( int i = 0; i < sockets_.Count(); ++i ) {
        ...
        if ( socket.isConnected() ) {
            ...
            const util::msectime_type ct = socket.getLastActivity() + core_.getConfig().getConnectTimeout();
            if ( ct <= currentTime ) {
                pendingSockets_.Push( &socket );
                rv = true;
...
void Connector::processEvents()
{
    ...
    while ( pendingSockets_.Count() > 0 ) {
        proto::Socket& socket = *pendingSockets_[0];
        smsc_log_debug( log_, "connecting socket %p ...", &socket );
        pendingSockets_.Delete(0);
        try {
            socket.connect();
            // move sockets to finishing
            smsc_log_info( log_, "socket %p connected", &socket );
            // finishingSockets_.Push(&channel);
            core_.registerSocket(socket);
            unregisterSocket(socket);
            continue;
        } catch ( std::exception& e ) {
            // cannot connect
            smsc_log_warn(log_,"cannot connect: %s", e.what());
            core_.handleError( socket, Exception( Status::NOT_CONNECTED,"cannot connect: %s",e.what()));
        }
    }
