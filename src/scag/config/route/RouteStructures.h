...
public:
  Route()
    ...
    transit(false), statistics(true), hideMessageBody_(false), srcSmeSystemId(),
  ...
  Route(const Route &r)
    ...
    transit(r.transit), statistics(r.statistics), hideMessageBody_(r.hideMessageBody_),
  ...
  Route(std::string routeId, bool archiving_, bool enabling_ /*acts as active*/, bool /*not used*/,
        bool istransit, bool hasStatistics, bool hideMessage, std::string srcSmeSystemId_, int32_t serviceId_,
  ...
  const bool hideMessageBody() const { return hideMessageBody_; }
