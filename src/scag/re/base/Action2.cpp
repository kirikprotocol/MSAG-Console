...
FieldType Action::CheckParameter( const SectionParams& params,
                                  PropertyObject& propertyObject,
                                  const char * actionName,
                                  const char * paramName,
                                  bool isRequired,
                                  bool isReadOnly,
                                  std::string& strParameter, bool& exist )
{
    const char * name = 0;
    ...
    if(ft == ftField) 
    {
        ...
        } else
        {
            if (!(at&atWrite)) 
              ...
              ( ft == ftConst || ft == ftUnknown || Session::isReadOnlyProperty(paramName) ))
