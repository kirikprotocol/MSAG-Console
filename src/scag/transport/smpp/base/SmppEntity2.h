...
public:
	...
	void putCommand(std::auto_ptr<SmppCommand>& cmd) {
		...
		case btRecvAndTrans:
			...
			smsc_log_error(log_,
					"Attempt to putCommand to RecvAndTrans enitity '%s'", info.systemId.c_str());
			break;
		case btTransceiver:
	...
	opid_type getSarMappingOpid(int sarmr, time_t currentTime) {
		...
		if (!ptr)
			...
			smsc_log_warn(log_,
					"too old mapping has been found tmo=%u, deleted", currentTime - ptr->creationTime);
	...
	bool setSarMappingOpid(int sarmr, opid_type opid) {
		...
		if (ptr) {
			...
			*ptr = SarData(opid, currentTime);
			mg.Unlock();
			smsc_log_debug(log_,
					"multipart mapping (sme/sar=%s/%x) replaced: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u -> opid=%u", getSystemId(), sarmr, old.dst ? old.dst->getSystemId() : "", old.opid, old.osar, old.count, old.total, opid);
			return true;
		} else {
			sarMapping_.Insert(sarmr, SarData(opid, currentTime));
			mg.Unlock();
			smsc_log_debug(log_,
					"multipart mapping (sme/sar=%s/%x) created: opid=%u", getSystemId(), sarmr, opid);
			return false;
		}
	}
	...
	void delSarMapping(int sarmr) {
		...
		smsc_log_debug(log_,
				"multipart sms mapping on sme=%s deleted: sar=%x", getSystemId(), sarmr);
	}
	...
	int countSlicedOnOutput(SmppEntity* dst, SmppCommand* cmd) {
		...
		if (replaced) {
			smsc_log_warn(log_,
					"multipart mapping (sme/sar=%s/%x) %s: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u -> %s/%u/%x/%u/%u dt=%u", getSystemId(), sarmr, what, old.dst ? old.dst->getSystemId() : "", old.opid, old.osar, old.count, old.total, dst->getSystemId(), opid, result, counted, lastIndex, unsigned(currentTime-oldTime));
			if (exception) {
				throw smsc::util::Exception(
			...
			smsc_log_debug(log_,
					"multipart mapping (sme/sar=%s/%x) %s: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u dt=%u", getSystemId(), sarmr, what, dst->getSystemId(), opid, result, counted, lastIndex, unsigned(currentTime-oldTime));
		}
		return result;
	}
	...
	SmppEntityErrors isEnabledAndBound() {
		...
		if (!info.enabled) {
			...
			smsc_log_debug(log_,
					"isEnabledAndBound SmppEntity systemId:%s enabled:false", info.systemId.c_str());
		} else if (bt == btNone) {
			ret_val = bind_btNone;
			smsc_log_debug(log_,
					"isEnabledAndBound SmppEntity systemId:%s SmppBindType=btNone", info.systemId.c_str());
		}
