...
private:
    ...
    struct SarData {
        ...
        time_t creationTime;
        SarData() :
                dst(0), opid(invalidOpId()), osar(0), count(0), total(0), creationTime(
                        0) {
        }
        SarData(opid_type theopid, time_t curTime) :
                dst(0), opid(theopid), osar(0), count(0), total(0), creationTime(
                        curTime) {
        }
    };
...
public:
    ...
    SmppEntity(const SmppEntityInfo& argInfo) {
        ...
        info = argInfo;
        {
            const char* cname = (
                    info.type == etSmsc ?
                            "sys.traffic.smpp.smsc" : "sys.traffic.smpp.sme");
            char buf[100];
            ...
            incCnt = mgr.registerAnyCounter(
                    new counter::TimeSnapshot(buf, 5, 100, o.get()));
            if (!incCnt.get()) {
                // smsc_log_error(log_,"entity '%s' cannot create counter '%s'",
                // info.systemId.c_str(),buf);
                throw exceptions::SCAGException(
                        "entity '%s' cannot create counter '%s'",
                        info.systemId.c_str(), buf);
            }
        }
        seq = 1;
        ...
        slicingSeq16 = 0;
        {
            ...
            counter::Manager& mgr = counter::Manager::getInstance();
            counter::ObserverPtr o = mgr.getObserver(cname);
            queueCount = mgr.registerAnyCounter(
                    new counter::Accumulator(buf, o.get()));
            if (!queueCount.get()) {
                throw exceptions::SCAGException(
                        "entity '%s' cannot create counter '%s'",
                        info.systemId.c_str(), buf);
    ...
    void putCommand(std::auto_ptr<SmppCommand>& cmd) {
        ...
        case btNone:
            throw smsc::util::Exception(
                    "Attempt to putCommand to unbound enitity '%s'",
                    info.systemId.c_str());
        case btReceiver:
        ...
        case btRecvAndTrans:
            ...
            smsc_log_error(log_,
                    "Attempt to putCommand to RecvAndTrans enitity '%s'",
                    info.systemId.c_str());
            break;
        case btTransceiver:
    ...
    int getUid() {
        ...
        case btNone:
            throw smsc::util::Exception(
                    "Failed to get uid of unbound entity '%s'",
                    info.systemId.c_str());
        case btReceiver:
        ...
        throw smsc::util::Exception("unknown bt=%d in getUid of '%s'", bt,
                info.systemId.c_str());
    }
    ...
    opid_type getSarMappingOpid(int sarmr, time_t currentTime) {
        ...
        if (!ptr)
            ...
            smsc_log_warn(log_,
                    "too old mapping has been found tmo=%u, deleted",
                    currentTime - ptr->creationTime);
    ...
    bool setSarMappingOpid(int sarmr, opid_type opid) {
        ...
        if (ptr) {
            ...
            *ptr = SarData(opid, currentTime);
            mg.Unlock();
            smsc_log_debug(log_,
                    "multipart mapping (sme/sar=%s/%x) replaced: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u -> opid=%u",
                    getSystemId(), sarmr, old.dst ? old.dst->getSystemId() : "",
                    old.opid, old.osar, old.count, old.total, opid);
            return true;
        } else {
            sarMapping_.Insert(sarmr, SarData(opid, currentTime));
            mg.Unlock();
            smsc_log_debug(log_,
                    "multipart mapping (sme/sar=%s/%x) created: opid=%u",
    ...
    int countSlicedOnOutput(SmppEntity* dst, SmppCommand* cmd) {
        ...
        bool exception = false;
        {
            ...
            if (!ptr) {
                what = "created";
                ptr = &sarMapping_.Insert(sarmr,
                        SarData(invalidOpId(), currentTime));
            }
            do {
        ...
        if (replaced) {
            smsc_log_warn(log_,
                    "multipart mapping (sme/sar=%s/%x) %s: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u -> %s/%u/%x/%u/%u dt=%u",
                    getSystemId(), sarmr, what,
                    old.dst ? old.dst->getSystemId() : "", old.opid, old.osar,
                    old.count, old.total, dst->getSystemId(), opid, result,
                    counted, lastIndex, unsigned(currentTime - oldTime));
            if (exception) {
                throw smsc::util::Exception(
                        "different dst found for multipart: dst1=%s dst2=%s",
                        old.dst ? old.dst->getSystemId() : "",
                        dst->getSystemId());
            }
        } else {
            smsc_log_debug(log_,
                    "multipart mapping (sme/sar=%s/%x) %s: dst/opid/sar/cnt/tot=%s/%u/%x/%u/%u dt=%u",
                    getSystemId(), sarmr, what, dst->getSystemId(), opid,
                    result, counted, lastIndex,
    ...
    bool checkSlicedDestination(uint32_t sarmr, SmppEntity* dst,
            time_t currentTime) {
        MutexGuard mg(sarMutex_);
        SarData* ptr = sarMapping_.GetPtr(sarmr);
        if (ptr && ptr->dst != 0) {
    ...
    enum SmppEntityErrors {
        ok = 0, disabled_route, bind_btNone
    };
    ...
    SmppEntityErrors isEnabledAndBound() {
        ...
        if (!info.enabled) {
            ...
            smsc_log_debug(log_,
                    "isEnabledAndBound SmppEntity systemId:%s enabled:false",
                    info.systemId.c_str());
        } else if (bt == btNone) {
            ret_val = bind_btNone;
            smsc_log_debug(log_,
                    "isEnabledAndBound SmppEntity systemId:%s SmppBindType=btNone",
                    info.systemId.c_str());
        }
    ...
    bool getAdminInfo(SmppEntityType entType, SmppEntityAdminInfo& ai) {
        smsc::core::synchronization::MutexGuard mg(mtx);
        if (info.type != entType)
            return false;
        ai.systemId = getSystemId();
        ai.bindType = bt;
        SmscSocket *pSmscSocket(0);
        std::string host;
        int port(0);
        switch (ai.bindType) {
        case btTransceiver:
    ...
    const char* checkAndBindChannel(SmppEntityType eType, SmppBindType bindType,
        ...
        smsc_log_debug(log_,
                "checkAndBindChannel SmppEntityType:%s SmppBindType:%s SmppChannel:%s Uid:%i Password:%s",
                SmppEntityTypeToString(eType).c_str(),
                SmppBindTypeToString(bindType).c_str(), ch->getPeer(), argUid,
                ((password) ? password : ""));
        MutexGuard mg(mtx);
        if (!info.enabled) {
            return "disabled";
        ...
        case btTransceiver:
            ...
            break;
        case btReceiver:
            recvChannel.reset(ch);
            break;
        case btTransmitter:
            transChannel.reset(ch);
            break;
        default:
            throw smsc::util::Exception("logic error: cant have bindType=%d",
                    bindType);
        }
        // info.host = get
        connected = true;
        ...
        case btNone:
            throw smsc::util::Exception(
                    "Failed to set uid of unbound entity '%s'",
                    info.systemId.c_str());
        case btReceiver:
            recvChannel->setUid(argUid);
            break;
    ...
    bool unbindChannel(SmppChannel* ch) {
        ...
        smsc_log_debug(log_, "unbindChannel %p %s bt=%d/%d", ch, ch->getSystemId(), bt, chbt);
        const int szmsg=50;
        char msg[szmsg]={0};
        bool ret_val=true;
        {
        ...
        if(!ret_val){
            smsc_log_warn(log_,"unbindChannel %s %s bt=%s/%s do not identical 0x%p"
                    ,msg ,ch->getSystemId() ,SmppBindTypeToString(bt).c_str() ,SmppBindTypeToString(chbt).c_str(),ch);
        }
        return ret_val;
    }
