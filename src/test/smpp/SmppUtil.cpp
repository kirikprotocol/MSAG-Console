		...
		case 2: //local time format (+)
			{
				...
				//дл€ Ќовосибирка export TZ=NSD-6, а timezone = -21600
...
{
	//если schedule_delivery_time = NULL, то немедленна€ доставка
...
bool SmppUtil::extractDataCoding(uint8_t dcs, uint8_t& dc)
{
	...
	if (!b[7])
	{
		...
		{
			dc = BINARY; //транслитерацию не производить, игнориру€ значение пофил€ абонента
			return true;
		}
	...
	else if (b[7] && b[6])
	{
		...
		if (b[5] && b[4])
		{
			...
			//(U)SIM-specific message игнорирую, т.к. транслитераци€ в этом случае невозможна
			return true;
		}
		else
		{
