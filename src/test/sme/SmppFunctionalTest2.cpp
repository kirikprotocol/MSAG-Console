...
void TestSmeFunc::executeCycle()
{
	...
	switch (seq[idx])
	{
		...
		case 1: //правильна€ sms
			...
			break;
		case 2: //правильна€ "сложна€" sms
			protocolTc.submitSmCorrectComplex(rand0(1), RAND_TC);
			break;
		case 3: //неправильна€ sms
		...
		case 101: //обновление настроек кодировки
			...
			break;
		case 102: //обновление настроек уведомлени€ о доствке
		...
		case 104: //обновление hide опции
			...
			break;
		case 105: //обновление профил€ некорректными данными
		...
		case 203:
			...
			break;
		*/
		case 301: //исключительно правильна€ sms
...
pair<uint32_t, time_t> TestSmeFunc::sendSmsResp(SmppHeader* header)
{
	...
	//на delivery receipt и сообщени€ от системных sme
...
int TestSmeErr::Execute()
{
	...
	while (!isStopping)
	{
		...
		sleep(1); //подождать, чтобы освободилс€ systemId
		if (paused)
		{
			cout << taskName() << " paused" << endl;
			pauseEvent.Wait();
	...
	int notConnected, int errSme, const string& smscHost, int smscPort)
{
	...
	//задание профилей (но только дл€ половины transceiver sme)
