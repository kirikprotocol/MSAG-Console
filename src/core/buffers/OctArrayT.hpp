...
typedef LWArrayExtension_T < uint8_t, uint16_t, LWArrayTraitsPOD_T<uint8_t> >
  OctArray16Iface;

template < uint16_t _StackSzArg >
class OctArray16_T : public LWArray_T<uint8_t, uint16_t, _StackSzArg, LWArrayTraitsPOD_T<uint8_t> > {
public:
  explicit OctArray16_T(uint16_t num_to_reserve = 0) 
    : LWArray_T<uint8_t, uint16_t, _StackSzArg, LWArrayTraitsPOD_T<uint8_t> >(num_to_reserve)
  { }
  ~OctArray16_T()
  { }
};
