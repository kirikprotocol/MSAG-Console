...
void SMSInitialDPArg::setIMSI(const smsc::util::IMSIString & imsi) throw(smsc::util::CustomException)
{
...
//requires the preceeding call of tzset()
void SMSInitialDPArg::setTPValidityPeriod(time_t vpVal, enum TP_VP_format fmt) throw(smsc::util::CustomException)
{
	...
	if ((res = packTimeT2BCD7((unsigned char (*)[7])(comp->_tPVP.buf), vpVal)) != 0)
	    throw smsc::util::CustomException(res, "IDPSmsArg: bad time value");
    }   break;
    default:
	throw smsc::util::CustomException("IDPSmsArg: unsupported TP-VP format: %u", (unsigned)fmt);
    }
    comp->idp.tPValidityPeriod = &(comp->_tPVP);
}

/* Sets VLR number and LocationNumber (duplicates it from VLR) */
void SMSInitialDPArg::setLocationInformationMSC(const TonNpiAddress& sadr) throw(smsc::util::CustomException)
{
    TonNpiAddress addr = sadr;
    /* NOTE: _vlrNumber may be only the ISDN INTERNATIONAL address */
    if (!addr.fixISDN())
        throw smsc::util::CustomException(-1, "IDPSmsArg: invalid VLR address",
                              addr.toString().c_str());
...
void SMSInitialDPArg::setLocationInformationMSC(const char* text) throw(smsc::util::CustomException)
{
    TonNpiAddress   sadr;
    if (!sadr.fromText(text))
        throw smsc::util::CustomException(-1, "IDPSmsArg: invalid VLR address",
...
void SMSInitialDPArg::setCellGlobalId(const CellGlobalId & cell_gid) throw(smsc::util::CustomException)
{
  if (!cell_gid.validate())
    throw smsc::util::CustomException(-1, "IDPSmsArg: invalid CellGlobalId",
...
void SMSInitialDPArg::encode(std::vector<unsigned char>& buf) const throw(smsc::util::CustomException)
{
    asn_enc_rval_t er;
