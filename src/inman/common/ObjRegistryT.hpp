...
public:
  ...
  //Destroys all objects in ascending order
  void clear(void)
  {
    while (!registry.empty()) {
      typename TRegistry::iterator it = registry.begin();
      if (it->second)
        delete it->second;
      registry.erase(it);
    }
  }
  //Destroys all objects in descending order
  void rclear(void)
  {
    while (!registry.empty()) {
      typename TRegistry::iterator it = --registry.end();
      if (it->second)
        delete it->second;
      registry.erase(it);
    }
