#pragma ident "$Id$"
/* ************************************************************************** *
 * Signalling Timer Watcher implementation.
 * ************************************************************************** */
...
#include "inman/common/TMWatcherDefs.hpp"
...
private:
    ...
    public:
        ...
        bool isRunning(void);
        bool Start(void);
        void Stop(void); //waits for last stopwatch being signaled
    };
...
class TimeWatchersRegistry {
protected:
    typedef std::map<SWTgtTime, TimeWatcherTMO*> TWRegistry;
...
public:
    ...
    ~TimeWatchersRegistry();

    void StopAll(void);

    //Returns started generic TimeWatcher
    TimeWatcher *    getTimeWatcher(uint32_t num_tmrs = 0);
    //Returns started TimeWatcherTMO responsible for given timeout
