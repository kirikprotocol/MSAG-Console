...
public:
    ...
    {
        Thread::Start();
        if (awaked.Wait(_TIMEOUT_STEP)) {
            smsc_log_error(logger, "%s: unable to start scheduler thread", _logId);
            return false;
        }
        return true;
    }

    //timeout set to zero cancels scheduling thread completion waiting 
    void Stop(unsigned timeOut_ms = _SHUTDOWN_TIMEOUT)
    {
        _sync.Lock();
        if (_state != schdStopped) {
            _state = schdStopping;
            smsc_log_debug(logger, "%s: stopping, timeout = %u ms ..", _logId, timeOut_ms);
            _sync.Unlock();
  
            if (timeOut_ms) {   //wait for 
                stopped.Wait(timeOut_ms);
                _sync.Lock();
                if (_state != schdStopped) {  //forcedly stop scheduling thread
