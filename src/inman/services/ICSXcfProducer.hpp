...
protected:
  explicit ICSProducerXcfAC(ICSUId ics_id) : ICSProducerAC(ics_id)
  { }

public:
  virtual ~ICSProducerXcfAC()
  { }

  //Resets XML config parser to a newly allocated one
  virtual ICSrvCfgReaderAC *newCfgReader(XMFConfig & xmf_cfg,
                                          const char * ics_sec,
...
public:
  explicit ICSProducerXcfAC_T(ICSUId ics_id) : ICSProducerXcfAC(ics_id)
  { }
  ...
  // -------------------------------------
  //Resets XML config parser to a newly allocated one
  ICSrvCfgReaderAC * newCfgReader(XMFConfig & xmf_cfg, const char *ics_sec,
                                  Logger * use_log = NULL)
  {
    xcfReader.reset(new _ReaderTArg(xmf_cfg, use_log, ics_sec));
  ...
  {
    if (!xcfReader.get() || (xcfReader->icsCfgState() < ICSrvCfgReaderAC::cfgPartial))
      return NULL;
