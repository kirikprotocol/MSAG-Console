 ...
 * ************************************************************************* */
#ifndef __SMSC_INMAN_CAPSM_TASK_HPP
#define __SMSC_INMAN_CAPSM_TASK_HPP

#include "inman/inman.hpp"
using smsc::inman::SS7_CFG;
...
public:
    ...
    CAPSmTaskAC(const TonNpiAddress & use_abn,
                const SS7_CFG & cfg_ss7, const INScfCFG * use_scf,
        ...
        , logPfx(log_pfx), logger(use_log)
    {
        _arg.reset(new InitialDPSMSArg(
            !idp_mode ? smsc::inman::comp::DeliveryMode_Originating :
...
public: 
    CAPSmTaskMT(const TonNpiAddress & use_abn,
                const SS7_CFG & cfg_ss7, const INScfCFG * use_scf,
                uint32_t serv_key, CAPSmMode idp_mode = idpMO, Logger * use_log = NULL)
        : CAPSmTaskAC(use_abn, cfg_ss7, use_scf, serv_key, idp_mode, (const char *)"CAPSmMT", use_log)
...
public: 
    CAPSmTaskSQ(const TonNpiAddress & use_abn,
                const SS7_CFG & cfg_ss7, const INScfCFG * use_scf,
                uint32_t serv_key, CAPSmMode idp_mode = idpMO, Logger * use_log = NULL)
        : CAPSmTaskAC(use_abn, cfg_ss7, use_scf, serv_key, idp_mode, (const char *)"CAPSmSQ", use_log)
    { 
...
#endif /* __SMSC_INMAN_CAPSM_TASK_HPP */
