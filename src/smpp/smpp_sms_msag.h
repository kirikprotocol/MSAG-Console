...
#if !defined __Cxx_Header__smpp_sms_msag_h__
#define __Cxx_Header__smpp_sms_msag_h__
...
inline void fillOptionalMsag(SmppOptional& optional,SMS* sms)
{
  ...
  if ( sms->hasBinProperty(Tag::SMPP_MESSAGE_PAYLOAD) )
  {
    ...
    {
      optional.set_messagePayload("",0); // Fixed: We need to set payload flag here!
...
inline bool fillSmppPduFromSmsMsag(PduXSm* pdu,SMS* sms)
{
  ...
  fillOptionalMsag(pdu->optional,sms);
  return true;
}

inline void fetchOptionalsMsag(SmppOptional& optional,SMS* sms)
{
  using namespace smsc::smeman;
  if ( optional.has_userMessageReference() )
    ...
    sms->setIntProperty(Tag::SMPP_USER_MESSAGE_REFERENCE,
                        optional.get_userMessageReference() & 0x7fffffff);
//  else
...
//    sms->setIntProperty(Tag::SMPP_USER_MESSAGE_REFERENCE,0);
  ...
  if(optional.has_ussd_session_id() && optional.has_ussdServiceOp())
  {
      // smpp+
      sms->setIntProperty(Tag::SMPP_USER_MESSAGE_REFERENCE,
                          optional.get_ussd_session_id() | 0x80000000);
...
inline bool fetchSmsFromSmppPduMsag(PduXSm* pdu,SMS* sms)
{
  using namespace smsc::smeman;
  __require__ ( pdu != NULL );
  __require__ ( sms != NULL );
 ...
 /* if ( pdu->header.comandId == DATA_SM )
  ...
  fetchOptionalsMsag(pdu->optional,sms);
  return true;
}

inline bool fetchSmsFromDataSmPduMsag(PduDataSm* pdu,SMS* sms)
{
  using namespace smsc::smeman;
...
inline bool fillDataSmFromSmsMsag(PduDataSm* pdu,SMS* sms)
{
  using namespace smsc::smeman;
  PduDataPartSm& data = pdu->get_data();
