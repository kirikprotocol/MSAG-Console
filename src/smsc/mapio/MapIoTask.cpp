...
extern "C" {
  ...
  USHORT_T Et96MapBindConf(ET96MAP_LOCAL_SSN_T lssn INSTANCEIDARGDEF(rinst), ET96MAP_BIND_STAT_T status)
  {
    ...
    smsc::snmp::SnmpCounter::SmeTrapSeverities trpSvrt=smsc::snmp::SnmpCounter::getInstance().getSmeSeverities(sid);
    char snmpMsg[1024];
    if(isBound)
    {
...
#ifdef EIN_HD
  reinit:
  if( isStopping || smsc::Smsc::getInstance().getStopFlag()) return;
  USHORT_T err;
  EINSS7CpMain_CpInit();
...
{
#ifdef SNMP
  smsc::snmp::SnmpCounter::SmeTrapSeverities smeTrpSvrt=smsc::snmp::SnmpCounter::getInstance().getSmeSeverities("MAP_PROXY");
...
void MapIoTask::killOverflow()
{
  ...
  while(!isStopping)
  {
    ...
    if(message.primitive==MAP_OPEN_IND)
    {
      ...
      smsc::Smsc::getInstance().incRejected();
    }
...
void MapIoTask::StartMap()
{
  try
  {
    ...
    do{
      sleep(1);
    }while(MAP_connectedInstCount==0 && !isStopping && !smsc::Smsc::getInstance().getStopFlag());
