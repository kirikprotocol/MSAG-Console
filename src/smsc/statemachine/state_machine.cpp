...
using smsc::snmp::SnmpCounter;
using smsc::common::RescheduleCalculator;
...
void StateMachine::sendNotifyReport(SMS& sms,MsgIdType msgId,const char* reason)
{
  try{
    if(sms.hasIntProperty(Tag::SMPP_USSD_SERVICE_OP) ||
       sms.getDeliveryReport()==REPORT_NOACK ||
       sms.getDeliveryReport()==smsc::profiler::ProfileReportOptions::ReportFinal ||
       sms.getIntProperty(Tag::SMSC_SUPPRESS_REPORTS))return;
                                           ...
                                           bool skipAttempt)
{
  ...
  try{
    ...
    */
    //time_t ntt=
    smsc->getScheduler()->RescheduleSms(id,sms,smsc->getSmeIndex(sms.dstSmeId));
...
void StateMachine::submitReceipt(SMS& sms,int type)
{
  ...
  try{
    ...
    if(has_route)
    {
      ...
      if(rr.info.destSmeSystemId!="MAP_PROXY")
      {
    ...
    */
    ...
    SmscCommand resp = SmscCommand::makeSubmitSmResp
                         (
                           ...
                           c.dialogId,
                           Status::OK,
                           c.t.command->dstNodeIdx,
                           c.t.command->sourceId,
