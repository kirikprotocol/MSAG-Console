#ifndef __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_DLPRCLISTRESP_HPP__
#define __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_DLPRCLISTRESP_HPP__
...
#ident "@(#) DlPrcListResp version 1.0"
...
public:
  ...
  std::string toString()const
  {
    ...
    if(resultFlag)
    {
      ...
      rv+="[";
      bool first=true;
      for(std::vector<PrcInfo>::const_iterator it=result.begin(),end=result.end();it!=end;it++)
      {
        if(first)
        {
          first=false;
  ...
  const std::vector<PrcInfo>& getResult()const
  {
    if(!resultFlag)
    {
  ...
  void setResult(const std::vector<PrcInfo>& value)
  {
    result=value;
    resultFlag=true;
  }
  ...
  void deserialize(DataStream& ds)
  {
    ...
    //{
    //  throw protogen::framework::IncompatibleVersionException("DlPrcListResp");
    //}
    //seqNum=ds.readInt32();
    while(!endOfMessage)
    {
      ...
      switch(tag)
      {
        ...
        default:
          ...
          //{
          //  throw protogen::framework::UnexpectedTag("DlPrcListResp",tag);
          //}
