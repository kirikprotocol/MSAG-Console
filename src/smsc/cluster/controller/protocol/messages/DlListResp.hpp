#ifndef __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_DLLISTRESP_HPP__
#define __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_DLLISTRESP_HPP__
...
#include "DlListInfo.hpp"


#ident "@(#) DlListResp version 1.0"
...
public:
  ...
  std::string toString()const
  {
    ...
    if(resultFlag)
    {
      ...
      rv+="[";
      bool first=true;
      for(std::vector<DlListInfo>::const_iterator it=result.begin(),end=result.end();it!=end;it++)
      {
        if(first)
        {
          first=false;
  ...
  const std::vector<DlListInfo>& getResult()const
  {
    if(!resultFlag)
    {
  ...
  void setResult(const std::vector<DlListInfo>& value)
  {
    result=value;
    resultFlag=true;
  }
  ...
  void deserialize(DataStream& ds)
  {
    ...
    //{
    //  throw protogen::framework::IncompatibleVersionException("DlListResp");
    //}
    //seqNum=ds.readInt32();
    while(!endOfMessage)
    {
      ...
      switch(tag)
      {
        ...
        case resultTag:
        {
          ...
          while(rd<len)
          {
            result.push_back(DlListInfo());
            result.back().deserialize(ds);
            rd+=result.back().length<DataStream>();
          }
          resultFlag=true;
        }break;
        ...
        default:
          ...
          //{
          //  throw protogen::framework::UnexpectedTag("DlListResp",tag);
          //}
