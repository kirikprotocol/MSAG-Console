#ifndef __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_ACLLISTRESP_HPP__
#define __GENERATED_MESSAGE_SMSC_CLUSTER_CONTROLLER_PROTOCOL_MESSAGES_ACLLISTRESP_HPP__
...
public:
  ...
  const std::vector<AclInfo>& getAclList()const
  {
    if(!aclListFlag)
    {
      throw protogen::framework::FieldIsNullException("aclList");
    }
    return aclList;
  }
  void setAclList(const std::vector<AclInfo>& value)
  {
    aclList=value;
    aclListFlag=true;
  }
    ...
    while(!endOfMessage)
    {
      ...
      switch(tag)
      {
        ...
        case aclListTag:
        {
          if(aclListFlag)
          {
            throw protogen::framework::DuplicateFieldException("aclList");
          }
...
protected:
  ...
  static const uint32_t aclListTag=2;
