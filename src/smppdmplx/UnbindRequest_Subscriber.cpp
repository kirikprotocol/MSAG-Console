...
smpp_dmplx::UnbindRequest_Subscriber::~UnbindRequest_Subscriber() {}
...
smpp_dmplx::UnbindRequest_Subscriber::handle(std::auto_ptr<smpp_dmplx::SMPP_message>& smpp, smsc::core_ax::network::Socket& socket)
{
  if ( smpp->getCommandId() == SMPP_message::UNBIND ) {
    ...
    smsc_log_info(dmplxlog,"UnbindRequest_Subscriber::handle::: got UNBIND message for processing from socket=[%s]. Message dump=[%s]", socket.toString().c_str(), unbindRequest->toString().c_str());
    ...
    if ( sessionSearchResult.first == true ) {
      ...
      smsc_log_info(dmplxlog,"UnbindRequest_Subscriber::handle::: Send UNBIND_RESPONSE message. Message dump=[%s]", unbindResponse->toString().c_str());
