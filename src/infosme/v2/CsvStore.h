#ifndef SMSC_INFOSME2_CSVSTORE_H
#define SMSC_INFOSME2_CSVSTORE_H
...
public:
  CsvStore( const std::string& argLoc ) : location(argLoc)
  {
    ...
    log=smsc::logger::Logger::getInstance("is2.csvs");
    unknownDirs=false;
    curDir=dirs.end();
  }
  ~CsvStore();
  ...
  uint64_t createMessage(time_t date,const Message& message,uint8_t state=MessageState::NEW);
  ...
  struct CsvFile{
    ...
    static void ReadRecord(smsc::core::buffers::File& f,Record& rec);
