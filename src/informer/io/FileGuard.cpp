...
void FileGuard::ropen( const char* fn )
{
    ...
    if (fn && fn[0]) {
        ...
        if (fd_==-1) {
            throw FileReadException(fn,errno,0,"ropen");
        }
    }
}
...
void FileGuard::create( const char* fn, mode_t mode, bool mkdirs, bool truncate )
{
    ...
    if (fn && fn[0]) {
        ...
        if (fd_==-1) {
            throw FileWriteException(fn,errno,"create(mode=%o,mkdirs=%d,trunc=%d)",mode,mkdirs,truncate);
        }
    }
}
...
const struct stat& FileGuard::getStat( struct stat& st ) const
{
    ...
    if (-1 == fstat(fd_,&st)) {
        throw FileReadException(fn_.c_str(),errno,0,"fstat");
    }
    return st;
}
...
void FileGuard::write( const void* buf, size_t buflen, bool atomic )
{
    ...
    while (buflen>0) {
        ...
        if (written == -1) {
            throw FileWriteException(fn_.c_str(),errno,"write");
        }
        buf = static_cast<const uint8_t*>(buf) + written;
        buflen -= written;
        pos_ += written;
        if (atomic && buflen>0) {
...
size_t FileGuard::read( void* buf, size_t buflen )
{
    ...
    while (buflen>0) {
        ...
        if (readlen==0) break; // EOF
            ...
            throw FileReadException(fn_.c_str(),errno,pos_,"read");
        }
        pos_ += readlen;
        wasread += readlen;
        buflen -= readlen;
        ...
        if ( -1 == stat(wk->c_str(),&dst) ) {
            if ( errno == ENOENT ) {
                ...
                if (-1 != mkdir(wk->c_str(),0777)) {
                    ...
                    throw FileWriteException(wk->c_str(),errno,"mkdir");
                }
            } else {
...
void FileGuard::truncate( size_t pos )
{
    if (fd_!=-1) {
        if (-1 == ftruncate(fd_,pos)) {
            throw FileWriteException(fn_.c_str(),errno,"truncate(pos=%llu)",ulonglong(pos));
        }
    }
}
