...
int ConfigWrapper::getInt( const char* name, int def, int min, int max, bool usedefault ) const
{
    ...
    try {
        ...
        if (log_) {
            smsc_log_warn(log_,"the parameter '%s' is not found, using %d", name, def);
        }
        val = def;
    }
    if (max>=min) {
        if (val<min) {
            if (log_) {
                smsc_log_warn(log_,"the parameter '%s' (%d) is less than %d, using %d",
                              name, val, min, min);
            }
            val = min;
        } else if (val>max) {
            if (log_) {
                smsc_log_warn(log_,"the parameter '%s' (%d) is greater than %d, using %d",
                              name, val, max, max);
            }
            val = max;
        }
