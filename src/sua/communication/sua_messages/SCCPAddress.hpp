...
class SCCPAddress {
public:
  explicit SCCPAddress(const sua_messages::TLV_GlobalTitle& gt);
  SCCPAddress(const sua_messages::TLV_PointCode& pc, const sua_messages::TLV_SSN& ssn);
  SCCPAddress(const sua_messages::TLV_GlobalTitle& gt, const sua_messages::TLV_SSN& ssn);
...
private:
  ...
  void transformGT2ITUFormat(const sua_messages::TLV_GlobalTitle& gt);
...
SCCPAddress::make_TLV_Address()
{
  ...
  if ( addressIndicator & 0x01 &&
    ...
    return TLV_ADDRESS(TLV_PointCode(ITU_PC(pcBuf)), TLV_SSN(addressValue[3]));
  } else {
