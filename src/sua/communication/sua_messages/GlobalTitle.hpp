...
public:
  ...
  GlobalTitle(nature_addr_t natureofAddress, const std::string& gtDigits);

  // GTI = 0x02 (global title includes translation type only)
  GlobalTitle(uint8_t translation_type, const std::string& gtDigits);

  // GTI = 0x03 (numbering plan and translation type are taken over; it is implicitly assumed 
  // that the Nature of Address = Unknown)
  GlobalTitle(uint8_t translation_type, numbering_plan_t numPlan, const std::string& gtDigits);
