...
GTTranslationTable::translate(const sua_messages::TLV_DestinationAddress& address)
{
  const sua_messages::GlobalTitle& gt = address.getGlobalTitle().getGlobalTitleValue();

  const std::string& gtValue = gt.getGlobalTitleDigits();

  const sua_messages::TLV_SSN& ssn = address.getSSN();
  uint8_t ssnValue=0;
  ...
  if ( routeWasFound )
    ...
    throw utilx::TranslationFailure(communication::NO_ADDR_TRANSLATION_FOR_SPECIFIC_ADDR, "GTTranslationTable::translate::: route not found");
  }
...
GTTranslationTable::getDefaultRoute()
{
  ...
  } else throw utilx::TranslationFailure(communication::NO_ADDR_TRANSLATION_FOR_SPECIFIC_ADDR, "GTTranslationTable::getDefaultRoute::: there isn't default route");
}
