...
LM_SGPLinks_AddLinkCommand::executeCommand()
{
  ...
  for(std::vector<std::string>::const_iterator iter = _localAddressList.begin(), end_iter = _localAddressList.end();
    ...
    runtime_cfg::RuntimeConfig::getInstance().notifyAddParameterEvent(runtime_cfg::CompositeParameter("config.sgp_links.link", _linkId), new runtime_cfg::Parameter("local_address", *iter));
  }

  oBuf << "local_port=[" << _localPort << "],";
  runtime_cfg::RuntimeConfig::getInstance().notifyAddParameterEvent(runtime_cfg::CompositeParameter("config.sgp_links.link", _linkId), new runtime_cfg::Parameter("local_port", _localPort));
  ...
  for(std::vector<std::string>::const_iterator iter = _remoteAddressList.begin(), end_iter = _remoteAddressList.end();
    ...
    runtime_cfg::RuntimeConfig::getInstance().notifyAddParameterEvent(runtime_cfg::CompositeParameter("config.sgp_links.link", _linkId), new runtime_cfg::Parameter("remote_address", *iter));
  }
  oBuf << "remote_port=[" << _remotePort << "]";
  runtime_cfg::RuntimeConfig::getInstance().notifyAddParameterEvent(runtime_cfg::CompositeParameter("config.sgp_links.link", _linkId), new runtime_cfg::Parameter("remote_port", _localPort));
