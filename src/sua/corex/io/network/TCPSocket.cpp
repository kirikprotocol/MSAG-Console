  ...
  : _toString_buf_init(false), _inputStream(NULL), _outputStream(NULL), _reverseSourceDestinationAddresses(false)
{
  ...
  inet_ntop(AF_INET, &_server_addr.sin_addr, _l_ip_address, sizeof(_l_ip_address));
}
...
TCPSocket::fillToStringInfo(bool needPeerAddr)
{
  ...
  inet_ntop(AF_INET, &addr.sin_addr, _l_ip_address, sizeof(_l_ip_address));
  ...
  if ( needPeerAddr ) {
    ...
    inet_ntop(AF_INET, &addr.sin_addr, _r_ip_address, sizeof(_r_ip_address));
...
#ifdef _REENTRANT
    if ( !(hp = gethostbyname_r(_host.c_str(), &h_result, addr_list_buffer, sizeof(addr_list_buffer), &h_errno_buf)) )
...
TCPSocket::setNoDelay(bool on)
{
  ...
  if ( setsockopt(_sockfd, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) < 0 )
