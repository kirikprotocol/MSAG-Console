...
public:
  ...
  { }
  BITSTRTiny_T(const uint8_t use_bits, uint8_t num_bits)
    : eyeline::util::BITArray_T<uint8_t, _StackBitsTArg>(use_bits, num_bits)
  { }
  BITSTRTiny_T(const BITSTRTiny_T & use_arr)
...
template <uint16_t _StackBitsTArg = _BITSTR_DFLT_SZ>
...
public:
  ...
  { }
  BITSTR_T(const uint16_t use_bits, uint16_t num_bits)
