...
test_OCTET_STRING_enc(char* err_msg)
{
  ...
  fprintf(logfile, "test_OCTET_STRING_enc:: OctsStrValue=%s, trSyntax=%s, retVal=%ld\n",
          utilx::hexdmp(buf4value, sizeof(buf4value)).c_str(), patternTrSyntax, retVal.encoded);
  TestPatternsRegistry::getInstance().insertResultPattern("test_OCTETSTRING", "01020304050504030201", patternTrSyntax);
