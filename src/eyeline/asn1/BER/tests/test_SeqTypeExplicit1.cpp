...
bool
test_SeqTypeExplicit1(char* err_msg)
{
  ...
  printf("test_SeqTypeExplicit1:\t\t");
  retVal = der_encode(&asn_DEF_SeqTypeExplicit1,
                      &value, write_transfer_syntax, patternTrSyntax);
  if (retVal.encoded == -1)
    return false;

  fprintf(logfile, "test_SeqTypeExplicit1:: SeqTypeExplicit1_t={a=0xEE,b=0x11,c=NULL}, trSyntax=%s, retVal=%ld\n",
  ...
  try {
    ...
    fprintf(logfile, "test_SeqTypeExplicit1:: ENCResult.status=%d\n", encResult.status);
    ...
    fprintf(logfile, "test_SeqTypeExplicit1:: SeqType1ExplicitValue={ 0xEE, 0x11, NULL}, trSyntax=%s\n",
            trSyntaxAsStr);
    if (strcmp(trSyntaxAsStr, patternTrSyntax)) {
      ...
      fprintf(logfile, "test_SeqTypeExplicit1:: expected value='%s', calculated value='%s'\n", patternTrSyntax, trSyntaxAsStr);
      return false;
    }
  } catch (std::exception& ex) {
    snprintf(err_msg, MAX_ERR_MESSAGE, "caught exception [%s]", ex.what());
    fprintf(logfile, "test_SeqTypeExplicit1::caught exception [%s]\n", ex.what());
    return false;
  }
  return true;
}
