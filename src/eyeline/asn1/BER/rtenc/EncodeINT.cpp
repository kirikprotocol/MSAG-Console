...
TSGroupBER::Rule_e EncoderOfINTEGER::setRule(TSGroupBER::Rule_e use_rule)
{
  if (_rule != use_rule) {
    _rule = use_rule;
    _tldSZO = 0;
...
TSLength  EncoderOfINTEGER::calculateVAL(bool do_indef/* = false*/) /*throw(std::exception)*/
{
...
//NOTE: Throws in case of value that cann't be encoded.
ENCResult EncoderOfINTEGER::encodeVAL(uint8_t * use_enc, TSLength max_len) const
{
