...
GTTranslationTable::translate(const communication::sua_messages::TLV_DestinationAddress& address)
{
  const communication::sua_messages::GlobalTitle& gt = address.getGlobalTitle().getGlobalTitleValue();

  const std::string& gtValue = gt.getGlobalTitleDigits();

  const communication::sua_messages::TLV_SSN& ssn = address.getSSN();
  uint8_t ssnValue=0;
  ...
  communication::LinkId outLinkSetId;
  bool searchResult;
  {
  ...
  if ( !searchResult ) {
    ...
    if ( outLinkSetId == communication::LinkId() )
      routeWasFound = false;
  }
  ...
  if ( routeWasFound )
    ...
    throw TranslationFailure(communication::NO_ADDR_TRANSLATION_FOR_SPECIFIC_ADDR, "GTTranslationTable::translate::: route not found");
  }
                                   ...
                                   communication::LinkId* outLinkSetId)
{
  ...
  std::map<uint8_t, communication::LinkId>::const_iterator iter = routeEntry->routeBySsn.find(destinationSSN);
...
GTTranslationTable::getDefaultRoute()
{
  ...
  } else throw TranslationFailure(communication::NO_ADDR_TRANSLATION_FOR_SPECIFIC_ADDR, "GTTranslationTable::getDefaultRoute::: there isn't default route");
}
...
void
GTTranslationTable::addDefaultTranslationEntry(const communication::LinkId& linkSetId)
{
