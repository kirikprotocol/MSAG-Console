...
void
CLCOMessageHandlers::prepareAndSendCLDRMessage(const communication::sua_messages::CLDTMessage& cldtMessage,
                                               const communication::LinkId& linkId,
                                               communication::return_cause_value_t returnCause)
{
  const communication::sua_messages::ProtocolClass& protocolClassValue = cldtMessage.getProtocolClass().getProtocolClassValue();
  if ( protocolClassValue.getRetOnErrorIndication() ) {
    communication::sua_messages::CLDRMessage cldrMessage(cldtMessage, returnCause);
    _cMgr.send(linkId, cldrMessage);
  }
}

void
CLCOMessageHandlers::handle(const communication::sua_messages::CLDTMessage& cldtMessage,
                            const communication::LinkId& linkId)
{
  ...
  try {
    ...
    if ( sua_user_communication::LinkSetInfoRegistry::getInstance().getAppId(outLinkSetId, &appId) ) {
      communication::libsua_messages::N_UNITDATA_IND_Message messageForSending(cldtMessage);
      ...
      const communication::sua_messages::TLV_SS7HopCount& ss7HopCount = cldtMessage.getSS7HopCount();
...
void
CLCOMessageHandlers::handle(const communication::sua_messages::CLDRMessage& cldrMessage,
                            const communication::LinkId& linkId)
{
  ...
  try {
    ...
    std::string appId;
    if ( sua_user_communication::LinkSetInfoRegistry::getInstance().getAppId(outLinkSetId, &appId) ) {
      communication::libsua_messages::N_NOTICE_IND_Message messageForSending(cldrMessage);
