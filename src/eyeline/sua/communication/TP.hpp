#ifndef __EYELINE_SUA_COMMUNICATION_TP_HPP__
# define __EYELINE_SUA_COMMUNICATION_TP_HPP__
...
struct TP {
  ...
  TP(int aPacketType, size_t aPacketLen, uint8_t* packetBodyPtr, size_t maxPcktLen)
    : packetType(aPacketType), packetLen(aPacketLen), maxPacketLen(maxPcktLen), packetBody(packetBodyPtr) {}
...
PreallocatedMemoryManager::getMemory<PreallocatedMemoryManager::MEM_FOR_PACKET>() {
  ...
  static sua::communication::TP tpObjectToGetItsSize;
