      ...
      void decode(uint8_t* buffer, int length)
      {
        ...
        for(int pos = 0; pos+4 < length; ++pos )
        {
          ...
          __require__(pos+len<=length);
					...
					__trace2__("Str: tag=%d key=%s len=%d pos=%d length=%d",tag,key?key:"NULL",len,pos,length);
					if ( key )
					{
      ...
      void decode(uint8_t* buffer, int length)
      {
        ...
        for(int pos = 0; pos+4 < length; ++pos )
        {
          ...
          string* key = tag_hash.getStrKeyForInt(tag);
					__trace2__("Int: tag=%d key=%s len=%d pos=%d length=%d",tag,key?key:"NULL",len,pos,length);
          if ( key )
          {
            __require__(len == 4);
