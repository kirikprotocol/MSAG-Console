...
void usage() {
  printf("Usage:   ss7trace modname traceon|traceoff\n");
  printf("Example: ss7trace ETSIMAP traceoff\n");
}
...
int verify(int argc, char* argv[]) {
  ...
  for(j = 0; j < sizeof(moduledesc)/sizeof(modesc); j++) {
    id = moduledesc[j].id;
    if (strcasecmp(argv[1],moduledesc[j].name) == 0) break;
  }
  if (id == 65535) {
  ...
  if (strcasecmp(argv[2],"traceon") != 0 &&
      strcasecmp(argv[2],"traceoff") !=0) {
    fprintf(stderr,"FAILED: unsupported operation %s\n",argv[2]);
    return -1;
  }
  return res;
}
int main(int argc, char* argv[]) {
  ...
  if ((res = ss7trace(argv[1],argv[2])) != 0) {
    deinitcp();
    return res;
  }
  return res;
...
int ss7trace(char* modname, char* operation) {
  ...
  for(j = 0; j < sizeof(moduledesc)/sizeof(modesc); j++) {
    id = moduledesc[j].id;
    if (strcasecmp(modname,moduledesc[j].name) == 0) break;
  }

  res = EINSS7_MgmtApiSendOrderReq(USERID, /*senderID*/
                                   MGMT_ID, /*receiverID*/
                                   id, /*moduleID*/
                                   strcasecmp(operation,"traceoff")?3:4, /*orderID = start all links*/
                                   0, /*lengthOfInfo*/
                                   NULL, /* orderInfo_p*/
                                   WAIT);

  if (res != EINSS7_MGMTAPI_RETURN_OK) {
    fprintf(stderr, "FAILED: trace order request. code %hu\n", res);
    return res ;
  }
