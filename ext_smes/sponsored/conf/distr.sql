...
deliverystat.save = INSERT INTO delivery_stats(subscriber_address, date, delivered, advertiser_id, sended) VALUES(?,?,?,?,?)
deliverystat.increase = UPDATE delivery_stats SET delivered=delivered + ?, sended=sended + ? WHERE subscriber_address=? AND date=? AND advertiser_id=?
deliverystat.aggregate = SELECT subscriber_address, advertiser_id, sum(delivered), sum(sended) FROM delivery_stats WHERE date >= ? AND date < ? GROUP BY subscriber_address,advertiser_id