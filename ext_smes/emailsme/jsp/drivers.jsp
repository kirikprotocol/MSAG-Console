...
<%
	...
	for (Iterator i = driverSectionNames.iterator(); i.hasNext();) {
		...
		%><tr class=row<%=(rowN++)&1%> id=dsdriver_row_<%=StringEncoderDecoder.encode(type)%>>
			...
			<td><%button(out, "/images/but_del.gif", "mbDel", "Remove", "Remove this datasource driver", "return clickRemoveDriver('dsdriver_row_" + StringEncoderDecoder.encode(type) + "');");%></td>
		</tr><%
	}
%>
<tr id=dsdriver_new class=row<%=(rowN++)&1%>>
	...
	<td><%button(out, "/images/but_add.gif", "mbAdd", "Add", "Add new datasource driver", "return clickAddDriver();");%></td>
</tr>
</table>
</div><%
page_menu_begin(out);
