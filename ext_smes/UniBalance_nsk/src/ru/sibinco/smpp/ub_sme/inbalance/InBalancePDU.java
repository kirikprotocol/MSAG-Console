...
public class InBalancePDU implements java.io.Serializable {
  ...
  protected final static int HEADER_LENGTH = 6;
  ...
  public InBalancePDU(InputStream is) throws InBalancePDUException, IOException {
    try {
      cmdId = (is.read() << 8) | is.read();
      requestId = (is.read() << 24) | (is.read() << 16) | (is.read() << 8) | is.read();
    } catch (IOException e) {
      throw e;
    } catch (Exception e) {
