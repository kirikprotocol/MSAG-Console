...
public class OutgoingQueue implements OutgoingQueueProxy {
  ...
  public OutgoingQueue(long submitTimeout, long maxRetry, long retryPeriod) {
    if (submitTimeout < 1000)
      if (Logger.isInfoEnabled()) Logger.info("Submit timeout too short (" + submitTimeout + "), should be 1000 or more. Use default submit timeout: 30000 millis.");
    else {
      if (Logger.isInfoEnabled()) Logger.info("Use " + submitTimeout + " submit timeout.");
      this.submitTimeout = submitTimeout;
    }
    if (retryPeriod < 1000)
      if (Logger.isInfoEnabled()) Logger.info("Retry period too short (" + retryPeriod + "), should be 1000 or more. Use default retry period: 30000 millis.");
    else {
      if (Logger.isInfoEnabled()) Logger.info("Use " + retryPeriod + " retry period.");
      this.retryPeriod = retryPeriod;
    }
    if (maxRetry < 1)
      if (Logger.isInfoEnabled()) Logger.info("Invalid max retries (" + maxRetry + "). Use default max retries: 1.");
    else {
  ...
  public synchronized void restoreOutgoingObject(int connectionId, int sequenceId) {
    ...
    if (idx > -1) {
      ...
      outQueue.add(0, obj);
      if (Logger.isDebugEnabled()) Logger.debug("OUT inserted in queue head by throttled error, queue size=" + outQueue.size() + "; sent size = " + sendTasks.size() + "; retry size = " + retryTasks.size());
      synchronized (sendNotifyMonitor) {
        sendNotifyMonitor.notifyAll();
      }
    } else {
  ...
  public synchronized void removeOutgoingObject(int connectionId, int sequenceId, int status) {
    ...
    if (status == PDU.STATUS_CLASS_NO_ERROR) {
      if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + connectionId + ";SN=" + sequenceId + ";AB=" + ab_addr + "] finalized, submit successful: " + status);
    } else if (status == PDU.STATUS_CLASS_PERM_ERROR) {
      if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + connectionId + ";SN=" + sequenceId + ";AB=" + ab_addr + "] finalized, permanent error received: " + status);
    } else {
      if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + connectionId + ";SN=" + sequenceId + ";AB=" + ab_addr + "] finalized, unexpected status received: " + status);
    }
  }
  ...
  private void clearQueue() {
    ...
    while (sendTasks.size() > 0) {
      ...
      if (obj.getSendTime() + submitTimeout < System.currentTimeMillis()) {
        ...
        if (obj.getRetries() < maxRetry) {
          if (obj.getOutgoingMessage().hasUssdServiceOp()) {
            if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + obj.getOutgoingMessage().getConnectionId() + ";SN=" + obj.getOutgoingMessage().getSequenceNumber() + "] cleared, submit timout reached but USSD could not be rescheduled");
          } else {
            retryTasks.add(new RetryTask(obj));
            if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + obj.getOutgoingMessage().getConnectionId() + ";SN=" + obj.getOutgoingMessage().getSequenceNumber() + "] rescheduled, attempt #" + obj.getRetries() + " submit timeout reached");
          }
        } else {
          if (Logger.isDebugEnabled()) Logger.debug("OUT [CID=" + obj.getOutgoingMessage().getConnectionId() + ";SN=" + obj.getOutgoingMessage().getSequenceNumber() + "] finalized, max attempts value was reached");
        }
      } else
        break;
    }
  ...
  public synchronized void reschedule() {
    ...
    if (Logger.isDebugEnabled()) Logger.debug("OUT QUEUE STATUS: out size=" + outQueue.size() + "; sent size = " + sendTasks.size() + "; retry size = " + retryTasks.size());
  }
