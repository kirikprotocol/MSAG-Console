...
import mobi.eyeline.mcahdb.engine.ds.impl.file.store.StoreImpl;
...
public class StoresCache {
  ...
  private static final SimpleDateFormat fileNameFormat = new SimpleDateFormat("yyyyMMddHH");
  ...
  public StoresCache(File storeDir) {
  ...
  public Collection<Store> getFilesForRead(Date from, Date till) throws DataSourceException {
    ...
    while(time <= tillTime) {
      Store s = getFile(getFileName(new Date(time)), true);
      if (s != null)
        result.add(s);      
      time += 3600000;
    }
