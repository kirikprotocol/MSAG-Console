...
public class IncomingQueue {
  ...
  public synchronized void initiateNewSession(Message message) {
    Log.debug("Initiate new session for address #" + message.getDestinationAddress() + "; abonent #" + message.getSourceAddress() + "; message=" + message.getMessageString());
    try {
      addIncomingObject(new IncomingObject(message));
    } catch (Exception e) {
      Log.warn("Could not initiate new session.", e);
    }
  }
