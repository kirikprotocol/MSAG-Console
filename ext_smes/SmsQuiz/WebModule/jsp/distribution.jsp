...
<jsp:useBean id="bean" scope="request" class="mobi.eyeline.smsquiz.beans.Distribution" />
<% if (!bean.isProcessed()) {%>
...
<tr class=row<%=rowN++&1%>>
  ...
  %><option value="<%=quizPathEnc%>" <%= (bean.isQuiz(quizPathEnc)) ? "selected":""%>><%=StringEncoderDecoder.encode(quizIdEnc)%></option>
<%}%></select></td>
