...
<tr class=row<%=rowN++&1%>>
  ...
  <td><input validation="nonEmpty" <%= distributionHelper.isReadOnly() ? "readonly" : "" %> class=txt name="<%=distributionHelper.getUid()+DistributionHelper.SA_FIELD%>"
             value="<%=StringEncoderDecoder.encode(distributionHelper.getSourceAddress())%>"></td>
</tr>
<tr class=row<%=rowN++&1%>>
  <th><%=getLocString("smsquiz.label.startTime")%></th>
  <td>
    <input validation="nonEmpty" <%= distributionHelper.isReadOnly() ? "readonly" : "" %> class=timeField id="<%=distributionHelper.getUid()+DistributionHelper.TIMEBEGIN_FIELD%>"
...
<tr class=row<%=rowN++&1%>>
  <th><%=getLocString("smsquiz.label.endTime")%></th>
    <td>
    <input validation="nonEmpty" <%= distributionHelper.isReadOnly() ? "readonly" : "" %> class=timeField id="<%=distributionHelper.getUid()+DistributionHelper.TIMEEND_FIELD%>"
...
<tr class=row<%=rowN++&1%>>
  ...
  <td><input class=calendarField <%= distributionHelper.isReadOnly() ? "readonly" : "" %> id="<%=distributionHelper.getUid()+DistributionHelper.DATEEND_FIELD%>"
...
<tr class=row<%=rowN++&1%>>
  ...
  <table>
    ...
    <col width="1%"><col width="32%">
    <tr>
      <td style="border:none;"><input <%= distributionHelper.isReadOnly() ? "readonly" : "" %> class=check type=checkbox name="<%=distributionHelper.getUid()+DistributionHelper.DAYS_FIELD%>"
...
<tr class=row<%=rowN++&1%>>
  ...
  <td><input <%= distributionHelper.isReadOnly() ? "readonly" : "" %> type=checkbox class=check value="true" id="<%=distributionHelper.getUid()+DistributionHelper.TXMODE_FIELD%>"
