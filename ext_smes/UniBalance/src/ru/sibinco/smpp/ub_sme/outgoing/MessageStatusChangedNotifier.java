            ...
            while (!notifications.isEmpty()) {
                ...
                if (listener == null) {
                    if (Logger.isInfoEnabled())
                        Logger.info(getName() + " could not find message status listener: " + n.getListenerName());
                } else {
                    listener.statusChanged(n.getMessageId(), n.getStatus());
                    if (Logger.isDebugEnabled())
                        Logger.debug(getName() + " notified message status listener: " + n.getListenerName() + ". MsgID=" + n.getMessageId() + "; Status=" + n.getStatus());
                }
            }
            synchronized (monitor) {
                if (!started) break;
