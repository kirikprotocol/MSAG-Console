...
public class MessageSender extends Thread implements ru.sibinco.smpp.ub_sme.util.ProductivityControllable {

  private final static Category Logger = Category.getInstance(MessageSender.class);
    ...
    while (true) {
      while ((obj = outQueue.getOutgoingObject()) != null && started) {
        try {
          ...
          // if connector not defined try to lookup connector by name from outgoing abonentRequest
          if (connector == null)
            connector = multiplexor.lookupConnectorByName(msg.getConnectionName());
          // if connector not defined try to lookup connector by source address of outgoing abonentRequest
          if (connector == null) {
          ...
          if (connector == null)
            throw new SMPPUserException(Data.ESME_RINVADR, "No route found for abonentRequest: source #" + msg.getSourceAddress() + "; destination #" + msg.getDestinationAddress());

          // try to get connector by new source address of outgoing abonentRequest
          if (connector == null) {
            String connectorName = msg.getSourceAddress();
            if (connectorName.lastIndexOf(":") > -1) {
              connectorName = connectorName.substring(connectorName.lastIndexOf(":") + 1);
          ...
          if (Logger.isInfoEnabled())
            Logger.info("Sending abonentRequest in async mode, address #" + msg.getSourceAddress() + "; abonent #" + msg.getDestinationAddress());
