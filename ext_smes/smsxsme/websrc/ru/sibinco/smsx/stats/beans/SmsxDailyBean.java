 ...
 */
public class SmsxDailyBean extends StatsBean{
 public static final int RESULT_RESULTS = StatsBean.PRIVATE_RESULT;
  ...
  protected int init(List errors) {
    ...
    tableHelper = new SmsxDailyTableHelper("smsx_daily", smsxContext.getStatRequestManager(), filter, smsxRequest.getId());
    return RESULT_OK;
  }
    ...
    try {
      ...
      if(requestId != null) {
        new SmsxWriter(smsxContext.getStatRequestManager()).
            writeWebDaily(out, requestId.intValue(), filter, smsxContext.getUserLocale(servletRequest.getUserPrincipal()));
      }
      out.flush();
    } catch (Exception e) {
      logger.error(e, e);
  ...
  public SmsxDailyTableHelper getTableHelper() {
    return tableHelper;
  }
