 ...
 */
public class SmsxTrafficBean extends StatsBean{
  ...
  private SmsxTrafficTableHelper tableHelper;

  private final SmsxTrafficFilter filter = new SmsxTrafficFilter();
  ...
  protected int init(List errors) {
    ...
    tableHelper = new SmsxTrafficTableHelper("smsx_traffic", smsxContext.getStatRequestManager(), filter, smsxRequest.getId());
    return RESULT_OK;
  }
    ...
    try {
      ...
      if(requestId != null) {
        new SmsxWriter(smsxContext.getStatRequestManager()).
            writeTraffic(out, requestId.intValue(), filter, smsxContext.getUserLocale(servletRequest.getUserPrincipal()));
      }
      out.flush();
    } catch (Exception e) {
      logger.error(e, e);
  ...
  public SmsxTrafficTableHelper getTableHelper() {
    return tableHelper;
  }
