...
<%@ page import="ru.sibinco.smsx.stats.beans.SmsxTrafficBean" %>
...
<%
  ...
  Set reports = bean.getReports();
	switch(beanResult)
	{
    case SmsxTrafficBean.RESULT_BACK:
      response.sendRedirect("index.jsp");
      return;
    case SmsxTrafficBean.RESULT_DOWNLOAD:
      response.sendRedirect(CPATH+"/esme_smsx/smsx_traffic_csv.jsp?requestId="+requestId+"&region=" + (bean.getRegion() == null ? "" :URLEncoder.encode(bean.getRegion())) +
          "&serviceId=" + (bean.getServiceId() == null ? "" : bean.getServiceId())+"&msc="+(bean.getMsc() == null ? "" : bean.getMsc()));
      return;
	}
%>
...
<div class=page_subtitle><%=getLocString("smsx.results.traffic.title")+" ["+bean.getFrom()+'-'+bean.getTill()+']'%></div>
