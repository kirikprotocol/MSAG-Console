...
secret.message.load.by.user = SELECT id, destination_address, message, source_address, send_date, dest_addr_subunit, status, save_delivery_status, smpp_status, notify_originator, conn FROM sec_messages WHERE status=0 AND destination_address=? ORDER BY send_date ASC
secret.message.load.count.by.user = SELECT count(*) FROM sec_messages WHERE status=0 AND destination_address=?
secret.message.load.by.id= SELECT id, destination_address, message, source_address, send_date, dest_addr_subunit, status, save_delivery_status, smpp_status, notify_originator, conn FROM sec_messages WHERE id=?
...
# Common requests
secret.load.user.and.messages = SELECT t1.password, t2.id, t2.destination_address, t2.message, t2.source_address, t2.send_date, t2.dest_addr_subunit, t2.status, t2.save_delivery_status, t2.smpp_status, t2.notify_originator, t2.conn FROM sec_users t1	LEFT JOIN sec_messages t2 ON (t1.address = t2.destination_address && t2.status=0) WHERE t1.address=?;