...
{
  private static final Logger logger = Logger.getLogger(GSM7CharSet.class.getName());
  ...
  public static String decode(byte in[]) throws UnsupportedEncodingException
  {
    ...
    for (int i = 0; i < in.length; ++i) {
      if ((in[i] & 0x7f) == 0x1b) {
        if (++i >= in.length) throw new UnsupportedEncodingException("incorrect input buffer, escape symbol is last in buffer");
        switch (in[i] & 0x7f) {
