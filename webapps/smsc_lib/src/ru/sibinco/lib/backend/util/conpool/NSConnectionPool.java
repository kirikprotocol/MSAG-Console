...
public class NSConnectionPool implements DataSource, ConnectionEventListener
{
  ...
  private long maxIdleTime = 86400000; // one day

  private int connectionsCount = 0;
  private int freeConnectionsCount = 0;

  private NSPooledConnection connections[];
  private NSPooledConnection freeConnections[];

  private boolean shutdown = false;
  private long maxShutdownTime = 600000;
  private IdleWatcher idleWatcher = null;
...
/*    if( maxConnections > minConnections ) {
  ...
  public void shutdown() throws SQLException
  {
    synchronized (connections) {
      ...
      while (connectionsCount > 0) {
        ...
        if ((System.currentTimeMillis() - waitStartedAt) > maxShutdownTime)
          throw new SQLException(
              "Connection pool couldn't shutdown in " + (maxShutdownTime / 1000) + " seconds. Not all connections were closed. Exiting shutdown procedure.");
  ...
  /**
   * Remove connection from connections list. Should be called from synchronized block.
   *
  ...
  protected NSPooledConnection popFreeConnection()
  {
    if (freeConnectionsCount > 0)
      return freeConnections[--freeConnectionsCount];
    else
      return null;
  }
    ...
    synchronized (connections) {
      ...
      if (lockedReport && logger.isDebugEnabled()) {
        ...
        for (int i = 0; i < connectionsCount; i++) {
          logger.warn("connection #" + i + " (" + connections[i].getConnection() + ") locked for " + (System.currentTimeMillis() - connections[i].lastUsed),
                      connections[i].trace);
        }
  ...
  /**
   * This function not implemented and will not be. Just create one more connection pool with required userid and password.
   */
