...
public class Buffer
{
  ...
  public boolean load(final View view, final boolean reload)
  {
    ...
    {
      if(file != null)
        modTime = jEdit.LongGet(filePath,jEdit.LastModified) ;

      // Only on initial load
      if(!reload && autosaveFile != null && jEdit.BoolGet(autosaveName,jEdit.Exists))
        loadAutosave = recoverAutosave(view);
      else
      {
        if(autosaveFile != null)
          jEdit.BoolGet(autosaveName,jEdit.Delete);//HttpGet(autosaveFile.getAbsolutePath(),jEdit.Delete);// autosaveFile.delete();
        loadAutosave = false;
      }
  ...
  public boolean saveAs(View view, boolean rename)
  {
    ...
    if(files == null)
      return false;
  System.out.println("Buffer.saveAs path= "+path+" rename= "+rename);
  ...
  public boolean save(final View view, String path, final boolean rename)
  {
    ...
    {
      long newModTime = jEdit.LongGet(filePath,jEdit.LastModified);
  ...
  public int checkFileStatus(View view)
  {
    ...
    if(!getFlag(IO) && !getFlag(LOADING) && file != null
            && !getFlag(NEW_FILE))
    {
      boolean newReadOnly = (jEdit.BoolGet(filePath,jEdit.Exists) && !jEdit.BoolGet(filePath,jEdit.CanWrite));
      ...
      long newModTime = jEdit.LongGet(filePath,jEdit.LastModified);//file.lastModified();

      if(newModTime != oldModTime)
      {
        modTime = newModTime;

        if(!jEdit.BoolGet(filePath,jEdit.Exists))//file.exists())
        {
  ...
  {
    methods=new HashMap(); // methods of this class
   ...
   // String[] foldModes = FoldHandler.getFoldModes();
    ...
    if(getProperty(ENCODING) == null)
      properties.put(ENCODING,new PropValue(jEdit.getSystemProperties().get("file.encoding"),false));//System.getProperty("file.encoding"),false));
    if(getProperty(LINESEP) == null)
      properties.put(LINESEP,new PropValue(jEdit.getSystemProperties().get("line.separator"),false));   //System.getProperty("line.separator")
  ...
  private HashMap methods; // methods of this class
  //}}}
  ...
  private boolean recoverAutosave(final View view)
  {
    ...
    final Object[] args = { autosaveName };
    int result = GUIUtilities.confirm(view,"autosave-found",args,
            JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE);

    if(result == JOptionPane.YES_OPTION)
    {
      VFSManager.getUrlVFS().load(view,this,autosaveName);
                            ...
                            boolean rename, boolean error)
  {
    ...
    {
      if(file != null)
        modTime = jEdit.LongGet(filePath,jEdit.LastModified);//file.lastModified();
