 ...
 * the {@link jEdit#addPluginJAR(Class)} and
...
*/
  ...
  public boolean checkDependencies()
  {
    ...
    while((dep = jEdit.getProperty("plugin." + name + ".depend." + i++)) != null)
    {
      ...
      else if(what.equals("plugin"))
      {
        ...
        else
        {
          ...
          jar.theseRequireMe.add(clas.getName());
          weRequireThese.add(jar.clas);
        }
      }
      else if(what.equals("class"))
      {
        if(!optional)
        {
          try
          {
            Class.forName(arg);//classLoader.loadClass(arg,false);
          }
          catch(Exception e)
          {
            String[] args = { arg };
    ...
    if(jars != null)
    {
      ...
      while(st.hasMoreTokens())
      {
        ...
        {
          weRequireThese.add(jarPath);   //todo this need change path to class name
          jar.theseRequireMe.add(clas.getName());
        }
      }
    }
  ...
  public void activatePlugin()
  {
    ...
    try
    {
      Class clazz =Class.forName(className);// classLoader.loadClass(className,false);
  ...
  public void activatePluginIfNecessary()
  {
    ...
    if(activate == null)
    {
      ...
      if(!jEdit.isMainThread())
      {
        ...
        jEdit.pluginError(clas.getName(),"plugin-error.not-42",null);
      }
      else
        activatePlugin();
    }
  ...
  public void deactivatePlugin(boolean exit)
  {
    ...
    if(!exit)
    {
      ...
      while(buffer != null)
      {
        ...
        /*&& buffer.getFoldHandler().getClass().getClassLoader() == classLoader*/)
        {
  ...
  public String toString()
  {
    if(plugin == null)
      ...
      return clas.getName() + ",class=" + plugin.getClassName();
  } //}}}
...
*/ // this.file = file;
  ...
  */  try
      ...
      catch(IOException io)
      {
        ...
        jEdit.pluginError(clas.getName(),"plugin-error.load-error",args);
  ...
  void uninit(boolean exit)
  {
    ...
    if(!exit)
    {
      ...
      while(iter.hasNext())
      {
        ...
        if(jar != null)
          jar.theseRequireMe.remove(this.clas.getName());
      }
...
// private File file;
  ...
  {
    Log.log(Log.WARNING,this,clas.getName() + " has an actions.xml but no plugin core class");
  ...
  private PluginCacheEntry generateCache(Class clas) throws IOException
  {
    ...
    PluginCacheEntry cache = new PluginCacheEntry();
    cache.modTime = System.currentTimeMillis();//file.lastModified();
    ...
    cache.actionsURI=clas.getResource("actions.xml");
    cache.browserActionsURI =clas.getResource("browser.actions.xml");
    dockablesURI = clas.getResource("dockables.xml");
    ...
    if (ClassName.equals("xml.XmlPlugin")) {
      Class comp=null;
      try {
        comp = Class.forName("xml.completion.XmlCompletion");
      } catch (ClassNotFoundException e) {
        e.printStackTrace();
      }
      if (comp!=null) {
      String keyG="xml.completion.glob.0.value";
      URL u=comp.getResource("ant-complete.xml");
      String valueG =u.toString();
      properties.put(keyG,valueG);
      String[] keysMode = {"asp","coldfusion", "html", "jhtml", "jsp" , "php" ,"shtml","velocity"};
      String[] valuesMode = {"html","html", "html", "jhtml", "ant" , "html" ,"html","html"};
      String[] valuesName = {"xsd","xsd","xsl","html","html"};

      for (int i = 0; i < valuesName.length; i++) {
        String key="xml.completion.namespace."+String.valueOf(i)+".value";
        String valueName =valuesName[i]+"-complete.xml";
        String value=comp.getResource(valueName).toString();
        properties.put(key,value);
      }
      for (int i = 0; i < valuesMode.length; i++) {
        String key="mode."+keysMode[i]+".xml.completion-info";
        String valueMode =valuesMode[i]+"-complete.xml";
        String value=comp.getResource(valueMode).toString();
        properties.put(key,value);
      }
      }
    }//  if (ClassName.equals("xml.XmlPlugin"))
