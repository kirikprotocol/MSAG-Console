...
public class TipOfTheDay extends EnhancedDialog
{
	...
	{
		String tipsName=MiscUtilities.constructPath(
			jEdit.getJEditHome(),"doc","tips");
    File[] tips = jEdit.FilesGet(tipsName,7); //7=ListFiles; //File[] tips = new File(tipsName).listFiles();
    ...
    try
		{	String r=jEdit.LongStringGet(tipsName+""+jEdit.separatorChar+tips[tipToShow].getPath(),jEdit.getParseXml());
      tipText.setText(r);// tipText.setPage(tips[tipToShow].toURL());
		}
		catch(Exception e)
		{
			e.printStackTrace();
