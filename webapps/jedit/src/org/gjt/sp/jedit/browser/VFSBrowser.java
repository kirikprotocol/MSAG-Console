...
public class VFSBrowser extends JPanel implements EBComponent, DefaultFocusComponent
{
  ...
  boolean multipleSelection, String position)
 {
  ...
  currentEncoding = jEdit.getProperty("buffer.encoding",
            (String)jEdit.getSystemProperties().get("file.encoding")) ;//System.getProperty("file.encoding"));
   ...
   else if(this.mode == BROWSER || this.mode == BROWSER_DIALOG)
   {
    ...
    if(_buffer == null)
    {
     ...
     props.put(Buffer.ENCODING_AUTODETECT, new Boolean(autoDetectEncoding));
      _buffer = jEdit.openFile(null,null, file.getPath(),false,props);
    }
  ...
  public void run()
  {
   ...
   if(node == null)
   {
    ...
    if(!pathField.getText().equals(path))
     pathField.setText(path);
    if(path.endsWith("/") ||
     path.endsWith(""+jEdit.separatorChar))
    {
