...
file_loop:   for(int i = 0; i < paths.length; i++)
 ...
 public static void loadMacros()
 {
  ...
  if(settings != null)
  {
   userMacroPath = MiscUtilities.constructPath(
    settings,"macros");
   loadMacros(macroHierarchy,"",userMacroPath);
  }
  ...
  if(jEdit.getJEditHome() != null)
  {
   ...
   loadMacros(macroHierarchy,"",systemMacroPath);
  }
 ...
 public static void recordTemporaryMacro(View view)
 {
  ...
  Buffer buffer = jEdit.openFile(null,settings + ""+jEdit.separatorChar
 ...
 static
 {
  ...
  protected static int IsDirectory=3;
  protected static int ListFiles=7;
 ...
 {  
  ...
  if(listFiles == null || listFiles.size() == 0)//if(macroFiles == null || macroFiles.length == 0)
   return;

  for(int i = 0; i < listFiles.size(); i++)
  {
      String fileName =(String)listFiles.get(i) ;//file.getName();
  // File file = new File(fileName);//macroFiles[i];

   if(jEdit.BoolGet(fileName,IsHidden))//file.isHidden())
   {
    /* do nothing! */
    continue;
   }
   else if(jEdit.BoolGet(fileName,IsDirectory))//file.isDirectory())
   {
    ...
    loadMacros(submenu,path + fileName + '/',fileName);
   }
   else
   {
    addMacro(fileName,path,vector);
   }
  }
 } //}}}
 ...
 private static void addMacro(String filePath, String path, Vector vector)
 {
  ...
  File file = new File(filePath);
    ...
    if (File.separatorChar!=jEdit.separatorChar)
        FileNamePath= FileNamePath.replace(File.separatorChar,jEdit.separatorChar);
    Handler handler = getHandlerForPathName(FileNamePath);
