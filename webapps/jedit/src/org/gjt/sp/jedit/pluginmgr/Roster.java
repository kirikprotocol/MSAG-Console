...
class Roster
{
 ...
 void performOperationsInAWTThread(Component comp)
 {
  ...
  {
   String pluginClassName = (String)toLoad.get(i);
      try {
        Class clas=Class.forName(pluginClassName);

      if(jEdit.getPluginJAR(pluginClassName) != null)
   {
    Log.log(Log.WARNING,this,"Already loaded: "
     + pluginClassName);
   }
   else
    jEdit.addPluginJAR(clas);
 ...
 class Remove extends Operation
 {
  ...
  private boolean deleteRecursively(File file)
  {
   ...
   if(jEdit.BoolGet(path,jEdit.getIsDirectory()))//file.isDirectory())
   {
        ...
        String[] children =jEdit.StringsGet(path,jEdit.getListFiles());// file.listFiles();
   ...
   ok &= jEdit.BoolGet(path,jEdit.getDelete());//file.delete();
  ...
  public void runInAWTThread(Component comp)
  {
   ...
   try
   {
    ...
    while(e.hasMoreElements())
    {
     ...
     String name = entry.getName().replace('/',jEdit.separatorChar);
