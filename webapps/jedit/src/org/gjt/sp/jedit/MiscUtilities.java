...
public class MiscUtilities
{
 ...
 public static String canonPath(String path)
 {
  ...
  if(path.startsWith("file://"))
   ...
   return path;
    if(jEdit.separatorChar == '\\')   //  if(jEdit.separatorChar == '\\')
  {
  ...
  if(path.startsWith("~" + ""+jEdit.separatorChar ))//File.separator))
  {
   path = path.substring(2);
   String home = jEdit.getJEditHome().toString();
      //String home = System.getProperty("user.home");

   if(home.endsWith(""+jEdit.separatorChar))//File.separator))
    return home + path;
   else
    return home + ""+jEdit.separatorChar+path;//File.separator + path;
  }
  else if(path.equals("~"))
   return jEdit.getJEditHome();//System.getProperty("user.home");
  else
   return path;
 } //}}}
...
//  {
    ...
    String canonPath= (String)jEdit.HttpGet(args,jEdit.getCanonPath()).get(0);
    if (null!=canonPath) return canonPath;
    else
    return path;
...
//  }
  ...
  public static String resolveSymlinksA(String path)
    {
      if(isURL(path))
        return path;
/*
 ...
 public static boolean isAbsolutePath(String path)
 {
  if(isURL(path))
   return true;
  else if(path.startsWith("~/") || path.startsWith("~" + ""+jEdit.separatorChar) || path.equals("~"))
   return true;
  ...
  public static boolean isAbsolutePathA(String path)
    {
      if(isURL(path))
        return true;
      else if(path.startsWith("~/") || path.startsWith("~" + ""+jEdit.separatorChar) || path.equals("~"))
 ...
 public static String constructPath(String parent, String path)
 {
  ...
  if(OperatingSystem.isDOSDerived())
  {
    ...
    && path.charAt(2) != '\\')
   {
    ...
    String dd = ".." +""+jEdit.separatorChar;  //+File.separator;
  String d = "."+ ""+jEdit.separatorChar;  //+File.separator;

  if(parent == null)
   parent = jEdit.getJEditHome();
 ...
 public static String concatPath(String parent, String path)
 {
  ...
  if (path.startsWith(""+jEdit.separatorChar))
   path = path.substring(1);
  else if ((path.length() >= 3) && (path.charAt(1) == ':'))
   path = path.replace(':', jEdit.separatorChar);

  if (parent == null)
   parent = System.getProperty("user.dir");

  if (parent.endsWith(""+jEdit.separatorChar)) //File.separator
   return parent + path;
  else
   return parent + ""+jEdit.separatorChar + path;  //File.separator
 } //}}}
 ...
 public static String getFileExtension(String name)
 {
  int fsIndex = Math.max(name.indexOf('/'),
   name.indexOf(jEdit.separatorChar));
  ...
  * @since jEdit 4.0pre1
  */
 public static void saveBackup(String path, int backups,
  String backupPrefix, String backupSuffix,
  String backupDirectory)
 {
  saveBackup(path,backups,backupPrefix,backupSuffix,backupDirectory,0);
 } //}}}
 ...
 {
    jEdit.SaveBackupGet(path,backups,backupPrefix,  backupSuffix,
...
/* public static boolean moveFile(File source, File dest)
 ...
 public static boolean pathsEqual(String p1, String p2)
 {
  ...
  if(p1.endsWith("/") || p1.endsWith(""+jEdit.separatorChar))//File.separator))
   p1 = p1.substring(0,p1.length() - 1);

  if(p2.endsWith("/") || p2.endsWith(""+jEdit.separatorChar))//File.separator))
...
loop:  for(;;)
 ...
 public static boolean isToolsJarAvailable()
 {
  ...
  if(toolsPath.toLowerCase().endsWith(""+jEdit.separatorChar + "jre"))
  ...
  if(jar == null)
  {
   ...
   Class clas=null;         //todo  will be maked zaglushka
      jEdit.addPluginJAR(clas); //toolsPath
  }
  else
   Log.log(Log.DEBUG, MiscUtilities.class,
