...
public abstract class VFS
{
 ...
 public String getFileName(String path)
 {
  ...
  int index = Math.max(path.lastIndexOf('/'),
   path.lastIndexOf(jEdit.separatorChar));
  if(index == -1)
   index = path.indexOf(':');
 ...
 public String getParentOfPath(String path)
 {
  ...
  while(lastIndex > 0
   && (path.charAt(lastIndex) == jEdit.separatorChar
  ...
  int index = path.lastIndexOf(jEdit.separatorChar,count);
  if(index == -1)
