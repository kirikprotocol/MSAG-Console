...
class SmsRowParser {
  ...
  private static final Logger logger = Logger.getLogger(SmsRowParser.class);
  ...
  static void parse(InputStream is, SmsRow row) {
      ...
      while (stream.available() > 0) {
        ...
        switch (tag) {
          ...
          case SMSC_MERGE_CONCAT_TAG: {
            ...
            row.addBodyParameter(tag, mcValue);
            break;
          }
          ...
          case SMPP_DATA_CODING_TAG: {
            ...
            row.addBodyParameter(tag, textEncoding);
            break;
          }
          case SMPP_ESM_CLASS_TAG: {
            esmClass = stream.readInt();
            row.addBodyParameter(tag, esmClass);
            break;
          }
          default: {
            switch (type) {
              ...
              default: {
                byte val[] = new byte[len];
                stream.read(val, 0, len);
                row.addBodyParameter(tag, "invalid tag type: " + StringEncoderDecoder.getHexString(val));
                break;
              }
            }
            break;
