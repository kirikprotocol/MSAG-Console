...
<%--                    <tr><td>--%>
        <c:forEach items="${bean.sites}" var="i">
            ...
            <c:set var="esite_eport" value="${esite}:${eport}"/>
            <c:set var="esite_sub" value="${fn:replace(esite,'.','_')}"/>
            <c:set var="sectHeader" value="sectionHeader_${esite_sub}:${eport}"/>
            <c:set var="esite_table" value="sitesp_table_${esite_sub}:${eport}"/>
...
<%--                            <div class="collapsing_tree_opened" id="${sectHeader}" style="width:100%;background-image:none">--%>
                    ...
                    <table id="${esite_table}" cellpadding="0" cellspacing="0" class="properties_list">
                        ...
                        <col width="99%">
                        <tr>
                            <td width="100%"><input type="radio" name="defaultSiteObjId" <c:if test="${(type=='route')?defaultSiteObjId==esite_eport:defaultSiteObjId}">checked</c:if> value="${esite}:${eport}">&nbsp;${fn:escapeXml(i.host)}:${fn:escapeXml(i.port)}</td>
                            <td align="left" nowrap="true"><input type="hidden" name="sitesHost"  id="${esite}:${eport}"
                                                                  value="${esite}">
                                <input type="hidden" name="sitesPort" value="${esite}:${eport}"></td>
                            <td><img src="content/images/but_del.gif"
<%--                                                 onClick="removeSection('${esite_sub}')"--%>
                                     onClick="removeSection('${esite_sub}:${eport}', '${sectHeader}', 'div_site_table')"
                                     style="cursor:pointer;"></td>
                        </tr>

                        <tr>
                            <td nowrap="true" align="right"><fmt:message>routes.edit.add.new.path</fmt:message>&nbsp;</td>
                            <td align="right"><input id="newPath_${esite_sub}:${eport}" class="txt"
                                                     name="newPath_${esite_sub}:${eport}" onkeyup="resetValidation(this)"></td>
...
<%--                                                 onclick="addPath(opForm.all.newPath_${esite_sub}, '${esite}', opForm.all.sitesp_table_${esite_sub}, '${esite_sub}')"--%>
                                     onclick="addPath('newPath_${esite_sub}:${eport}', '${esite}:${eport}', 'sitesp_table_${esite_sub}:${eport}', '${esite_sub}:${eport}')"
                                     style="cursor:pointer;"></td>
                        </tr>
                        <c:set var="rowN" value="0"/>
                        <c:forEach items="${i.pathLinks}" var="j">
                            <c:set var="epath" value="${fn:escapeXml(j)}"/>
                            <tr class="row${rowN%2}" id="pathRow_${esite_sub}:${eport}:${epath}">
                                <td></td>
                                <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${epath}<input type="hidden"
                                                                                 name="pathLinks"
                                                                                 value="${esite}:${eport}^${epath}">
                                </td>
                                <td><img src="content/images/but_del.gif"
<%--                                                     onClick="removeRow(opForm.all.sitesp_table_${esite_sub}, 'pathRow_${esite_sub}_${epath}')"--%>
                                         onClick="removeRow('sitesp_table_${esite_sub}:${eport}', 'pathRow_${esite_sub}:${eport}:${epath}')"
                                         style="cursor:pointer;"></td>
                            </tr>
