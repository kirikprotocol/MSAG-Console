...
<c:choose>
 ...
 <sm-ep:property title="${head}" >
  <input class=check type=checkbox name="${name}" id="id_${fn:escapeXml(name)}" value="true" <c:if test="${bean[name]}">checked</c:if>>&nbsp;<label for="id_${fn:escapeXml(name)}"><fmt:message>${title}</fmt:message></label>
 </sm-ep:property>
</c:when>
<c:otherwise>
  <input class=check type=checkbox name="${name}" id="id_${fn:escapeXml(name)}" value="true" <c:if test="${bean[name]}">checked</c:if>>&nbsp;<label for="id_${fn:escapeXml(name)}"><fmt:message>${title}</fmt:message></label>
