...
<c:set var="subjectItems" value="${bean.subjectItems}"/>
...
<input type="hidden" id="mbHttpSubjEdit" name="mbHttpSubjEdit" value="">
<input type="hidden" id="httpSubjId" name="httpSubjId" value="">
<input type="hidden" id="subjectType" name="subjectType" value="">
<script type="text/javascript">



    function editSubj(idToEdit) {
        opForm.mbHttpSubjEdit.value = idToEdit;
        opForm.httpSubjId.value = idToEdit;
        opForm.subjectType.value = '0';
        opForm.action = "<%=request.getContextPath() + (request.getServletPath().endsWith(".jsp")
                         ...
                         : request.getServletPath())%>/edit.jsp?editId=" + idToEdit;
        opForm.submit();
        return false;
    }
</script>
...
<table class="list" cellspacing="0">
    ...
    <tbody>
       <c:forEach var="user" items="${subjectItems}" varStatus="status">
                           ...
                           <td>
                               <c:choose>
                                   <c:when test="${edit == column}">
                                       <a href="#" onclick="return editSubj('${Id}');">${itemValue}</a>
                                   </c:when>
                                   <c:otherwise>
                                       ${itemValue}
                                   </c:otherwise>
                       ...
                       <c:otherwise>
                           ...
                           <td>
                               <c:if test="${edit == column}">
                                   <a href="#" onClick="return editSubj('${itemValue}');"></c:if>
                               <c:choose>
