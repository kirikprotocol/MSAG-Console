...
//        appContext.getSmppManager().createUpdateMetaEndpoints(super.getLoginedPrincipal().getName(), meta, isAdd(), appContext);
        appContext.getSmppManager().createUpdateMetaEndpoints( userName, meta, oldMetaSvc, isAdd(), appContext, Constants.STORE_TYPE_SVC );
        updateServices();
        ...
        for( Iterator iterator = smes.keySet().iterator(); iterator.hasNext(); ){
            ...
            if( svc != null ){
                try {
                    ...
                    appContext.getSmppManager().createUpdateServicePoint( userName,
                            svc, false, svc.isEnabled(), appContext, svc, true);
