    ...
    <table class=properties_list cellspacing=0 height="60">
        <tr class=row0>
            ...
            <td nowrap><input type=text id="fromDate" name="fromDate" class=calendarField
                              value="<%=bean.getFromDate()%>" maxlength=20 style="z-index:22;"><button
                    class=calendarButton type=button onclick="return showCalendar(fromDate, false, true);">...</button>
            </td>
            <th><%=getLocString("common.util.TillDate")%>:</th>
            <td nowrap><input type=text id="tillDate" name="tillDate" class=calendarField
                              value="<%=bean.getTillDate()%>" maxlength=20><button class=calendarButton type=button
                                                                                   onclick="return showCalendar(tillDate, false, true);">
                ...</button></td>
            <% if (needCSVDownload) { %>
            <td nowrap>
                <input class=check type=checkbox id=includeErrors
                       name=includeErrors <%=bean.isIncludeErrors() ? "checked" : ""%>>
            </td>
...
<%
    ...
    if (stat != null && !needCSVDownload) {
        ...
        java.util.Collection dates = stat.getDateStat();
%>
    ...
    <td colspan=8>
        <table class=list cellspacing=0 cellpadding=0 border=0>
            ...
            <td width="23%" align=right>
                &nbsp;&nbsp;&nbsp;<%= ((hour.getHour() < 10) ? "0" + hour.getHour() : "" + hour.getHour())%></td>
