...
<%@ page import="ru.novosoft.smsc.jsp.smsstat.SmsStatFormBean,
                 ru.novosoft.smsc.jsp.smsstat.StatExportBean" %>
...
<div class=content>
    ...
    <table class=properties_list cellspacing=0>
        ...
        <tr class=row<%= rowN%>>
            ...
            <td width="80%" nowrap><input type=text id="date" name="date" class=calendarField
                                          value="<%=bean.getDate()%>" maxlength=10 style="z-index:22;"><button
                    class=calendarButton type=button onclick="return showCalendar(date, false, false);">...</button>
            </td>
        </tr>
    </table>
    <br/>
    ...
    <table class=properties_list cellspacing=0>
        <tr class=row<%= (rowN++) % 2%>>
            ...
            <td nowrap><input
                    ...
                    value="<%= StatExportBean.DEFAULT_DESTINATION%>"
            ...
            <td nowrap><input
                    class=radio type="radio" name="destination" id=destinationUser
                    value="<%= StatExportBean.USER_DESTINATION%>"
            <%= (bean.getDestination() == StatExportBean.USER_DESTINATION) ? "checked":""%>
                    onclick="javascript:userDefined()">
        ...
        <tr class=row<%= (rowN++) % 2%>>
            ...
            <td nowrap><input class=txt type=password id="password" name="password" value="<%=bean.getPassword()%>">
            </td>
        </tr>
        <tr class=row<%= (rowN++) % 2%>>
            <th><%=getLocString("stat.export.tablePrefix")%></th>
            <td nowrap><input class=txt type=text id="tablesPrefix" name="tablesPrefix"
                              value="<%=bean.getTablesPrefix()%>"></td>
        </tr>
    </table>
</div> <!-- content -->
<%
