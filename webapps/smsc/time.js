...
function timeClose(){
	...
	document.all.timeIFrame.runtimeStyle.display="none";
	timeMouseDown=false;
...
function createTimePanel(hour,minute,second,PM) {
  ...
  if(timeUS)if(timeCurPM) timeAMPMsv.innerHTML="PM"; else timeAMPMsv.innerHTML="AM";
...
//		alert(dtHour+"\n"+dtMinute+"\n"+dtSecond+"\n"+dt);
  ...
  if (timeUS) {hour%=12;if (hour==0) hour=12;}
  timeCurHour = hour;
  timeCurMinute = minute;
  timeCurSecond = second;
  timeCurPM = PM;
//-------------------------------------------------
	var PageX=0;var PageY=0;var E=timeInputText;
	while(E!=null){
...
function timeMD(){
    ...
    if (timeUS&&a.name.indexOf("timeAMPMsv")!=-1) {
      if(a.innerHTML=="AM") {timeCurPM=true;a.innerHTML="PM";} else {timeCurPM=false;a.innerHTML="AM";}
      timeActivePanel.value=timeMakeTimeStr(timeCurHour,timeCurMinute,timeCurSecond,timeCurPM);
    } else if(a.name.indexOf("timeTime")!=-1){
      if (a.name.indexOf("Ok")!=-1) return timeClose(); else
      if (a.name.indexOf("Hour")!=-1){
        if (a.name.indexOf("Up")!=-1) timeCurHour = (timeCurHour + 1)%(timeUS?12:24); else timeCurHour = (timeCurHour + 23)%(timeUS?12:24);
      ...
      if (a.name.indexOf("Minute")!=-1){
        if (a.name.indexOf("Up")!=-1) timeCurMinute = (timeCurMinute + 1)%60; else timeCurMinute = (timeCurMinute + 59)%60;
        timeTimeMinute.innerHTML = (timeCurMinute<10?"0":"")+timeCurMinute;
        timeActivePanel.value=timeMakeTimeStr(timeCurHour,timeCurMinute,timeCurSecond,timeCurPM);
        timePressedTime=true;
      ...
      if (a.name.indexOf("Second")!=-1){
        if (a.name.indexOf("Up")!=-1) timeCurSecond = (timeCurSecond + 1)%60; else timeCurSecond = (timeCurSecond + 59)%60;
        timeTimeSecond.innerHTML = (timeCurSecond<10?"0":"")+timeCurSecond;
        timeActivePanel.value=timeMakeTimeStr(timeCurHour,timeCurMinute,timeCurSecond,timeCurPM);
        timePressedTime=true;
