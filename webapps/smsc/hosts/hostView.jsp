...
<%@ page import="ru.novosoft.smsc.jsp.PageBean,
                 ru.novosoft.smsc.jsp.smsc.hosts.HostView,
                 java.util.Arrays,
                 java.util.List" %>
...
<div class=content>
    ...
    <table class=list cellspacing=1 width="100%">
        ...
        <tbody>
            ...
            %>
            <tr class=row<%=row & 1%>>
                <td><input class=check type=checkbox name=serviceIds
                           value="<%=encodedServiceId%>" <%=checkedServices.contains(serviceId) ? "checked" : ""%>></td>
                <td><a href="javascript:editService('<%=encodedServiceId%>')"
                       title="<%=getLocString("host.editServHint")%>"><%=getLocString("common.buttons.edit")%></a></td>
                <td><a href="javascript:viewService('<%=encodedServiceId%>')"
                       title="<%=getLocString("host.viewServInfo")%>"><%=encodedServiceId%></a></td>
