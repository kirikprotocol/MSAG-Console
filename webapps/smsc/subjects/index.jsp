...
<div class=content>
    ...
    <table id=filterMasksTable>
        ...
        <tr id=filterMaskRow_<%= filterMaskHex %>>
            <td>
                <input class=txt name=filter_masks value="<%= StringEncoderDecoder.encode(filterMask) %>"
                       validation="mask" onkeyup="resetValidation(this)">
            </td>
...
<thead>
    <tr>
        ...
        <th>
            <a href="javascript:setSort('Default SME')" <%= bean.getSort().endsWith("Default SME") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : "" %>
               title="<%= getLocString("subjects.sortDefaultSmeHint") %>"><%= getLocString("subjects.sortDefaultSme") %></a>
        </th>
        <th>
            <a href="javascript:setSort('Masks')" <%= bean.getSort().endsWith("Masks") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : "" %>
               title="<%= getLocString("common.sortmodes.masksHint") %>"><%= getLocString("common.sortmodes.masks") %></a>
        </th>
    </tr>
</thead>
<tbody>
    ...
    <tr class=row<%= row & 1 %>>
        <td class=check>
            <input class=check type=checkbox name=checkedSubjects
                   value="<%= encName %>" <%= bean.isSubjectChecked(name) ? "checked" : "" %>>
        </td>
        <td <%= onClick %>>
            <div id=<%= rowId %>_HEAD <%= encNotes.length() > 0 ? "class=collapsing_list_closed" : "class=collapsing_list_empty" %>>
                <a href="javascript:edit('<%= encName %>')"
                   title="<%= getLocString("subjects.editSubTitle") %>"><%= encName %></a>
            </div>
        </td>
        <td <%= onClick %>>
            <%= encDefSme %>
