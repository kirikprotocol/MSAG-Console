...
<jsp:useBean id="bean" class="ru.novosoft.smsc.jsp.smsc.providers.IndexProvider"/>
...
<%
    ...
    switch (bean.process(request)) {
        ...
        case IndexProvider.RESULT_FILTER:
            response.sendRedirect("providersFilter.jsp");
            return;
...
<hr>
<table class=list cellspacing=0 cellpadding=0 id=PROVIDER_LIST_TABLE>
    <col width="1%">
    ...
    <tbody>
        <%{
            int row = 0;
            boolean have = true;
            QueryResultSet providers = (QueryResultSet) bean.getProviders();
            try {
            ...
            if (have) {
                for (Iterator i = providers.iterator(); i.hasNext(); row++) {
                    DataItem item = (DataItem) i.next();
                    String providerId = (String) item.getValue("id");
                    String encProviderId = StringEncoderDecoder.encode(providerId);
                    String rowId = "ROUTE_ID_" + StringEncoderDecoder.encodeHEX(providerId);
                    String name = (String) item.getValue("name");
                    String encName = StringEncoderDecoder.encode(name);
        %>
        <tr class=row<%=row & 1%>>
            <td><input class=check type=checkbox name=checkedProviderNames
                       value="<%=encName%>" <%=bean.isProviderChecked(name) ? "checked" : ""%>></td>
            <td><%=encProviderId%>&nbsp;</td>
            <td><a href="javascript:edit('<%=encProviderId%>','<%=encName%>')"
                   title="<%=getLocString("providers.editSubTitle")%>"><%=encName%></a></td>
        </tr>
