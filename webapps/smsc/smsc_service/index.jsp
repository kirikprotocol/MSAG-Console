  ...
  startSection(out, "profiler", "Profiler", false);
    ...
    startSection(out, "profiler.ussdOpsMapping", "USSD operations mapping", false);
      startParams(out);
        ...
        param(out, "translit on",                               "profiler.ussdOpsMapping.TRANSLIT ON",         bean.getIntParam("profiler.ussdOpsMapping.TRANSLIT ON"));
    ...
    startSection(out, "profiler.notify", "Profile changes notification", false);
      startParams(out);
        param(out, "Agent host",                                "profiler.notify.host",         	             bean.getStringParam("profiler.notify.host"));
        param(out, "Agent port",                                "profiler.notify.port",         	             bean.getIntParam("profiler.notify.port"));
        param(out, "Cache directory",                           "profiler.notify.dir",         	               bean.getStringParam("profiler.notify.dir"));
      finishParams(out);
    finishSection(out);
  finishSection(out);
  //################################## abonentinfo #############################
