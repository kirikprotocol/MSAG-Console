...
<%!
    ...
    private static final StringBuffer createOption(StringBuffer result, final String priority, final String catPriority) {
      return result.append("<option value=\"").append(priority).append('"').append(priority.equals(catPriority)
      ? " selected>"
      : ">").append(priority).append("</option>");
    }
    ...
    private final void printCategory(JspWriter out, Logging.LoggerCategoryInfo category) throws IOException {
      startSectionPre(out, category.getFullName(), category.getName(), category.isRoot(), createSelection(
              Logging.catParamNamePrefix, category.getFullName(), category.getPriority()));
      ...
      {
        {
          ...
          for (Iterator i = category.getChilds().values().iterator(); i.hasNext(); ) {
            ...
            if (!child.hasChilds()) {
              ...
              paramSelectPre(out, child.getName(), Logging.catParamNamePrefix + child.getFullName(), Arrays.asList(PRIORITIES
                      ), child.getPriority(), null, "font-size:80%;");
            }
          }
