...
<tr class=row<%=(rowN++)&1%>>
    ...
    <input class=check type=checkbox name=roles id=role01 value="apply"             <%=bean.isUserInRole("apply")             ? "checked" : ""%>>&nbsp;<label for=role01>Apply config changes</label><br>
		...
		<input class=check type=checkbox name=roles id=role07 value="smsView_archive"   <%=bean.isUserInRole("smsView_archive")   ? "checked" : ""%>>&nbsp;<label for=role07>SMS View Archive only</label><br>
		<input class=check type=checkbox name=roles id=role08 value="smsView_operative" <%=bean.isUserInRole("smsView_operative") ? "checked" : ""%>>&nbsp;<label for=role08>SMS View Opertaive only</label><br>
		<input class=check type=checkbox name=roles id=role09 value="smsstat"           <%=bean.isUserInRole("smsstat")           ? "checked" : ""%>>&nbsp;<label for=role09>SMS Statistics</label><br>
		<input class=check type=checkbox name=roles id=role10 value="mscman"            <%=bean.isUserInRole("mscman")            ? "checked" : ""%>>&nbsp;<label for=role10>MSCs</label><br>
		<input class=check type=checkbox name=roles id=role11 value="users"             <%=bean.isUserInRole("users")             ? "checked" : ""%>>&nbsp;<label for=role11>Users</label><br>
		<input class=check type=checkbox name=roles id=role12 value="routes"            <%=bean.isUserInRole("routes")            ? "checked" : ""%>>&nbsp;<label for=role12>Routes</label><br>
		<input class=check type=checkbox name=roles id=role13 value="subjects"          <%=bean.isUserInRole("subjects")          ? "checked" : ""%>>&nbsp;<label for=role13>Subjects</label><br>
		<input class=check type=checkbox name=roles id=role14 value="aliases"           <%=bean.isUserInRole("aliases")           ? "checked" : ""%>>&nbsp;<label for=role14>Aliases</label><br>
		<input class=check type=checkbox name=roles id=role15 value="profiles"          <%=bean.isUserInRole("profiles")          ? "checked" : ""%>>&nbsp;<label for=role15>Profiles</label><br>
		<input class=check type=checkbox name=roles id=role16 value="hosts"             <%=bean.isUserInRole("hosts")             ? "checked" : ""%>>&nbsp;<label for=role16>Hosts</label><br>
		<input class=check type=checkbox name=roles id=role17 value="services"          <%=bean.isUserInRole("services")          ? "checked" : ""%>>&nbsp;<label for=role17>Services</label>
    <%
