...
<div class=content>
    ...
    <table class=properties_list cellspacing=0 cellspadding=0>
        ...
        <tr class=row<%=(rowN++) & 1%>>
            ...
            <td><input title="<%=getLocString("common.hints.filter")%>" class=txt name=loginFilter
                       value="<%=bean.getLoginFilter()%>"></td>
        </tr>
        <tr class=row<%=(rowN++) & 1%>>
            <th><%=getLocString("common.sortmodes.firstName")%>:</th>
            <td><input title="<%=getLocString("common.hints.filter")%>" class=txt name=firstNameFilter
                       value="<%=bean.getFirstNameFilter()%>"></td>
        </tr>
        <tr class=row<%=(rowN++) & 1%>>
            <th><%=getLocString("common.sortmodes.lastName")%>:</th>
            <td><input title="<%=getLocString("common.hints.filter")%>" class=txt name=lastNameFilter
                       value="<%=bean.getLastNameFilter()%>"></td>
        </tr>
        <tr class=row<%=(rowN++) & 1%>>
            <th><%=getLocString("common.sortmodes.department")%>:</th>
            <td><input title="<%=getLocString("common.hints.filter")%>" class=txt name=departmentFilter
                       value="<%=bean.getDepartmentFilter()%>"></td>
        </tr>
    </table>
</div>
<%
