    ...
    <table class=list cellspacing=0 width="100%">
        ...
        <thead>
            <tr>
                ...
                <th>
                    <a href="javascript:setSort('login')" <%=bean.getSort().endsWith("login") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("common.sortmodes.loginHint")%>"><%=getLocString("common.sortmodes.login")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('firstName')" <%=bean.getSort().endsWith("firstName") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("common.sortmodes.firstNameHint")%>"><%=getLocString("common.sortmodes.firstName")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('lastName')" <%=bean.getSort().endsWith("lastName") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("common.sortmodes.lastNameHint")%>"><%=getLocString("common.sortmodes.lastName")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('dept')" <%=bean.getSort().endsWith("dept") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("common.sortmodes.departmentHint")%>"><%=getLocString("common.sortmodes.department")%></a>
                </th>
            </tr>
        </thead>
        <tbody>
            ...
            %>
            <tr class=row<%=row & 1%>>
                <td><input class=check type=checkbox name=checkedUserLogins
                           value="<%=encLogin%>" <%=bean.isLoginChecked(login) ? "checked" : ""%>></td>
                <td><a href="javascript:edit('<%=encLogin%>')"
                       title="<%=getLocString("users.editSubTitle")%>"><%=encLogin%></a></td>
                <td><%=encFirstName%>&nbsp;</td>
                <td><%=encLastName%>&nbsp;</td>
                <td><%=encDept%>&nbsp;</td>
            </tr>
