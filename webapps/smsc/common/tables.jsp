  ...
  throws java.io.IOException
{
  ...
  {
    out.println("  <th class=\"list\" width=\"" + (99/columns.length)+ "%\">"
	            + "<a href=\"?sort=" + (sort.equals(columns[i])
				                             ? URLEncoder.encode("-" + columns[i])
											 : URLEncoder.encode(columns[i])) + "\">"
				+ StringEncoderDecoder.encode(columns[i])
				+ "</a></th>");
  }
  ...
  for (Iterator i = results.iterator(); i.hasNext(); )
  {
    ...
    for (int j = 0; j < columns.length; j++)
	{
      ...
      if (value instanceof Vector)
	  {
        ...
        {
          out.println(  StringEncoderDecoder.encode((String)k.next())
		              + (k.hasNext() ? "<br>" : ""));
        }
        out.println("</td>");
      }
	  ...
	  {
	  	out.println("    <td class=\"list\" style=\"width:1%\">"
		                + "<input disabled type=\"Checkbox\" "
						            + (((Boolean)value).booleanValue() ? "checked" : "")
						+ "/></td>");
        params +=   URLEncoder.encode(columns[j])
		          + '='
		          + URLEncoder.encode(((Boolean)value).toString())
				  + '&';
	  }
	  else
	  {
