...
{
	return "<a href=\"" + trigger_url + getTriggerParamName(trigger_name)+ "=" 
			+ (process_trigger(trigger_name, session, request)
				? ("false\">" + StringEncoderDecoder.encode(trigger_to_false))
				: ("true\">" + StringEncoderDecoder.encode(trigger_to_true))
			  )
			+ "</a>";
}

String show_set_combo(String name, Collection values, String defaultValue)
{
	String result = "<select name=\""+StringEncoderDecoder.encode(name)+"\">";
	for (Iterator j = values.iterator(); j.hasNext(); )
	{
		String value = (String) j.next();
		result += "<option" + (value.equals(defaultValue) ? " selected>" : ">") 
				+ StringEncoderDecoder.encode(value) + "</option>";
	}
	result += "</select>";
	return result;
}
