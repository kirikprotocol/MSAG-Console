...
<%@ page import="ru.novosoft.smsc.admin.smsview.SmsQuery,
...
<tr class=row1>
    ...
    <td><input class=txt type="text" id="abonentAddress" name="abonentAddress" value="<%=bean.getAbonentAddress()%>"
               size=25 maxlength=25 onkeyup="hideAddresses()"></td>
    <th><%=getLocString("smsview.smeId")%>:</th>
    <td><input class=txt type="text" id="smeId" name="smeId" value="<%=bean.getSmeId()%>" size=17 maxlength=15
               onkeyup="hideSmeIds()"></td>
</tr>
<tr class=row0>
    <th><%=getLocString("smsview.sourceAddress")%>:</th>
    <td><input class=txt type="text" id="fromAddress" name="fromAddress" value="<%=bean.getFromAddress()%>" size=25
               maxlength=25></td>
    <th><%=getLocString("smsview.sourceSmeId")%>:</th>
    <td><input class=txt type="text" id="srcSmeId" name="srcSmeId" value="<%=bean.getSrcSmeId()%>" size=17 maxlength=15>
    </td>
</tr>
<tr class=row1>
    <th><%=getLocString("smsview.destAddress")%>:</th>
    <td><input class=txt type="text" id="toAddress" name="toAddress" value="<%=bean.getToAddress()%>" size=25
               maxlength=25></td>
    <th><%=getLocString("smsview.destSmeId")%>:</th>
    <td><input class=txt type="text" id="dstSmeId" name="dstSmeId" value="<%=bean.getDstSmeId()%>" size=17 maxlength=15>
    </td>
</tr>
<tr class=row0>
...
<tr class=row1>
    ...
    <td nowrap>
        <select name="lastResult"><%int lastResult = bean.getLastResult();%>
            ...
            %><option
                value="<%= errorValue.errorCode%>" <%= (lastResult == errorValue.errorCode) ? "selected" : ""%>><%=
...
<tr class=row0>
    ...
    <td nowrap><input type=text id=fromDate name=fromDate class=calendarField value="<%=bean.getFromDate()%>"
    ...
    <td nowrap><input type=text id="tillDate" name="tillDate" class=calendarField value="<%=bean.getTillDate()%>"
                      maxlength=20>
...
<tr class=row1>
    ...
    <td nowrap>
        <input class=check type=checkbox name=exactRowsCount
               id=exactRowsCount <%=bean.isExactRowsCount() ? "checked" : ""%>>
    ...
    <table class=list cellspacing=0>
            ...
            <tr class=row0>
                ...
                <th>
                    <a href="javascript:setSort('name')" <%=bean.getSort().endsWith("name") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.idHint")%>"><%=getLocString("common.sortmodes.id")%></a></th>
                <th>
                    <a href="javascript:setSort('sendDate')" <%=bean.getSort().endsWith("sendDate") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.sortSubmitHint")%>"><%=getLocString("smsview.sortSubmit")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('lastDate')" <%=bean.getSort().endsWith("lastDate") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.sortTriedHint")%>"><%=getLocString("smsview.sortTried")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('from')" <%=bean.getSort().endsWith("from") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.sourceHint")%>"><%=getLocString("common.sortmodes.source")%></a>
                </th>
                <th>
                    <a href="javascript:setSort('to')" <%=bean.getSort().endsWith("to") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.destinationHint")%>"><%=getLocString("common.sortmodes.destination")%></a>
                </th>
                <th><%=getLocString("smsview.route")%></th>
                <th>
                    <a href="javascript:setSort('status')" <%=bean.getSort().endsWith("status") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                       title="<%=getLocString("smsview.statusHint")%>"><%=getLocString("common.sortmodes.status")%></a>
                </th>
            </tr></thead>
        <tbody><%
        ...
        %><tr class=row<%=rowN&1%>0>
            <td nowrap valign=top>
                <input class=check type=checkbox name=checkedRows
                       value="<%= smsRowId%>" <%=bean.isRowChecked(smsRowId) ? "checked" : ""%>>
            </td>
            <td nowrap valign=top style="text-align: right">
                <a target="smsc_smsview" href="<%= CPATH%>/smsview/view.jsp?viewId=<%= smsRowId%>&mbView=show"
                   title="<%=getLocString("smsview.viewDetailSubTitle")%>"><%= smsRowId%></a><br>
                <!--%= row.getAttempts()%-->
            </td>
...
<% if (bean.getTotalSizeInt() > 0) {
    page_menu_begin(out);
    page_menu_button(session, out, "mbSave", "common.buttons.save", "smsview.saveToFileHint");
    page_menu_space(out);
    page_menu_end(out);
} %>
</div>
