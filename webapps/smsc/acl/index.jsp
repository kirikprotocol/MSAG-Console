...
<div class=content>
    ...
    <select name="aclId">
        ...
        %><option
            value="<%=acl.getId()%>"<%= aclInfo != null && bean.getAclId() == acl.getId() ? " selected" : ""%>><%=acl.getName()%></option><%
        }
    %>
    </select>
</div><%
...
<div class="content">
    ...
    <table class=list cellspacing=0 cellspadding=0>
        ...
        <tr class=row1>
            ...
            <td><input class=txt name="name" value="<%=aclInfo.getName()%>" validation="nonEmpty"
...
%><div class="content">
    ...
    <table class=list cellpadding="0" cellspacing="0" id="addresses_table">
        ...
        <td><%=address%></td>
        <td><%
            delButton(out, "remove address", "remove address", "acl.delButtonHint", "delAclAddress(" + row + ")");%></td>
    </tr><%
                row++;
            }
        }
