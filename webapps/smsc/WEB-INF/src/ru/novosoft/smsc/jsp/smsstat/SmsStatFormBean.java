...
public class SmsStatFormBean extends IndexBean {
    ...
    public int process(HttpServletRequest request) {
        ...
        if (mbQuery != null) {
            try {
                ...
                journalAppend(SubjectTypes.TYPE_statview, "Stat query", Actions.ACTION_VIEW, "from date:" + dateFrom, "till date:" + dateTill);
            } catch (Exception exc) {
                statistics = null;
                exc.printStackTrace();
            ...
            if (statistics != null) {
                this.setFromDate((String) session.getAttribute("dateFrom"));
        ...
        if (mbDetail != null) {
            try {
                mbDetail = null;
                journalAppend(SubjectTypes.TYPE_statview, "Detail By Routes", Actions.ACTION_VIEW);
    ...
    private void exportCounters(JspWriter out, CountersSet set) throws IOException {
        out.print(set.accepted);
        out.print(COL_SEP);
        out.print(set.rejected);
        out.print(COL_SEP);
        out.print(set.delivered);
        out.print(COL_SEP);
        out.print(set.failed);
        out.print(COL_SEP);
        out.print(set.rescheduled);
        out.print(COL_SEP);
        out.print(set.temporal);
        out.print(COL_SEP);
        out.print(set.peak_i);
        out.print(COL_SEP);
        out.print(set.peak_o);
        out.println();
    }
    ...
    public int exportStatistics(HttpServletRequest request, JspWriter out) {
        ...
        journalAppend(SubjectTypes.TYPE_statview, "CSV Download", Actions.ACTION_LOAD);
        return RESULT_OK;
