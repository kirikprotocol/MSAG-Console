    ...
    private int updateFilter() {
        try {
            ...
            if (queryMask != null) {
                ...
                byte oldHide = aliasFilter.getHide();
                aliasFilter.setAliases(new String[]{queryMask});
                aliasFilter.setAddresses(new String[0]);
                aliasFilter.setHide(AliasFilter.HIDE_NOFILTER);
                QueryResultSet addressByAlias = appContext.getSmsc().getAliases().query(new AliasQuery(pageSize, preferences.getAliasesFilter(), preferences.getAliasesSortOrder(), startPosition));
                if (addressByAlias.size() > 0) {
                    String[] sourceMasks = new String[addressByAlias.size() + 1];
                    String[] destMasks = new String[addressByAlias.size() + 1];
                    for (int i = 0; i < addressByAlias.size(); i++) {
                        sourceMasks[i] = (String) addressByAlias.get(i).getValue("Address");
                        destMasks[i] = (String) addressByAlias.get(i).getValue("Address");
                    }
                    sourceMasks[addressByAlias.size()] = queryMask;
                    destMasks[addressByAlias.size()] = queryMask;
                    routesFilter.setSourceMaskStrings(sourceMasks);
                    routesFilter.setDestinationMaskStrings(destMasks);
                } else {
                ...
                if (querySubj == null) {
                    ...
                    subjFilter.setMasks(new String[]{queryMask});
