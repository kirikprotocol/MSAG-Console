...
public class IndexProvider extends IndexBean {
    ...
    public int process(final HttpServletRequest request) {
        ...
        if (request.getSession().getAttribute("PROVIDER_NAME") != null) {
            providers = getProvidersByName((String) request.getSession().getAttribute("PROVIDER_NAME"));
            request.getSession().removeAttribute("PROVIDER_NAME");
        }
