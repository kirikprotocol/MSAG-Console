    ...
    public Statistics getStatistics(StatQuery query) throws AdminException
    {
       ...
       // System.out.println("Start scanning statistics at: " + new Date());
        ...
        for (Iterator iterator = selectedFiles.keySet().iterator(); iterator.hasNext() && !finished;)
        {
            ...
            {
            //  System.out.println("Parsing file: "+dateDayFormat.format(fileDate)+" GMT ("+dateDayLocalFormat.format(fileDate)+" local)");
            }
            ...
            try
            {
                ...
                Date lastDate = null; Date curDate = null;
                int prevHour = -1;
                ...
                while (true) // iterate file records (by minutes)
                {
                    try
                    {
                        ...
                        int hour = readUInt8(input); int min = readUInt8(input);
                        ...
                        if(fromQueryDate != null && curDate.getTime() < fromQueryDate.getTime()) {
                          skipBytes(input, rs1 - 2);
                         // System.out.println("Hour: "+hour+" skipped");
                          continue;
                        }
                        ...
                        if(tillQueryDate != null && curDate.getTime() >= tillQueryDate.getTime()) {
                          finished = true; break; // finish work
                        }
      ...
      System.out.println("End scanning statistics at: " + new Date()+" time spent: "+(System.currentTimeMillis()-tm)/1000);


      DateCountersSet dateCounters = null;  Date lastDate = null;
