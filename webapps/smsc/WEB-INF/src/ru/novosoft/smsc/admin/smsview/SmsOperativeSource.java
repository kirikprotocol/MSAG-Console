...
public class SmsOperativeSource  extends SmsSource
{
  ...
  public boolean querySelect(SmsQuery query,Message responce)
  {
    ...
    else { if (needExpression(query.getSrcSmeId()) && needSmsExpression(sms.getSrcSmeId())) {
    ...
    if (query.getFromDateEnabled()) { //list.add("SUBMIT_TIME >=?");
       ...
       String SubmitTime=formatter.format(sms.getSubmitTime());
       String FromDate=formatter.format(query.getFromDate());
       Date date=sms.getSubmitTime();
       int compare=sms.getSubmitTime().compareTo(query.getFromDate());
