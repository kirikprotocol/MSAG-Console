...
public class Console extends Thread {
    ...
    public Console(SMSCAppContext context)
        ...
        if (context != null) {
            ...
            try {
                inactivityTimeout = cfg.getInt("console.timeout");
            }
    ...
    public void close() {
        ...
        while (sessions.size() > 0) {
            ...
            try {
                session.farewell(" Console server stopped");
                sleep(500);
            } catch (Exception e) {
            }
            session.close(true);
    ...
    public void run() {
        try {
            while (!isStopping) {
                synchronized (sessionMonitor) {
                    try {
                        sessionMonitor.wait(inactivityTimeout / 2);
                    }
                    catch (InterruptedException e) {
                        break;
                    }
                    ...
                    for (int i = 0; i < sessions.size(); i++) {
                        ...
                        if (session.isTimedOut(inactivityTimeout)) {
                            ...
                            try {
                                session.farewell(" Session is timed out.");
                                sleep(500);
                            } catch (Exception e) {
                            }
    ...
    public ClosedGroupManager getClosedGroupManager() {
        return context.getClosedGroupManager();
    }
