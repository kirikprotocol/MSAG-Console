...
public class RsSmsMessage extends Message
{
  ...
  public void receive(InputStream bis) throws IOException
  {
    ...
    try {
      sms.setOriginatingAddress(Message.readString8(is));
    } catch (AdminException e) {
      throw new IOException(e.getMessage());
    }
    try {
      sms.setDestinationAddress(Message.readString8(is));
    } catch (AdminException e) {
      throw new IOException(e.getMessage());
    }
    try {
      sms.setDealiasedDestinationAddress(Message.readString8(is));
    } catch (AdminException e) {
      throw new IOException(e.getMessage());
    }
