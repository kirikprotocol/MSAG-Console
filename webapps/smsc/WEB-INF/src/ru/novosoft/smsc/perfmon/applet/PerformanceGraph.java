    ...
    public void paint(Graphics gg) {
        ...
        if (snaps.size() > 1) {
            ...
            for (int i = 0; i < viewableGraph && idx > 0; i++, cntg++) {
                ...
                // draw submit graphs
                drawGraphLine( g, posy, posx, (int)snap.last[PerfSnap.IDX_SUBMITERR], (int)prevSnap.last[PerfSnap.IDX_SUBMITERR], 0, 0, maxheight, colorGraphSubmitErr);
                drawGraphLine( g, posy, posx, (int)snap.last[PerfSnap.IDX_RETRY], (int)prevSnap.last[PerfSnap.IDX_RETRY], (int)snap.last[PerfSnap.IDX_SUBMITERR], (int)prevSnap.last[PerfSnap.IDX_SUBMITERR], maxheight, colorGraphRetry);
                drawGraphLine( g, posy, posx, (int)snap.last[PerfSnap.IDX_SUBMIT], (int)prevSnap.last[PerfSnap.IDX_SUBMIT], (int)(snap.last[PerfSnap.IDX_SUBMITERR]+snap.last[PerfSnap.IDX_RETRY]), (int)(prevSnap.last[PerfSnap.IDX_SUBMITERR]+prevSnap.last[PerfSnap.IDX_RETRY]), maxheight, colorGraphSubmit);
                //draw deliver graphs
                drawGraphLine( g, posy, posx, (int)snap.last[PerfSnap.IDX_DELIVERERR], (int)prevSnap.last[PerfSnap.IDX_DELIVERERR], 0, 0, maxheight, colorGraphDeliverErr);
                drawGraphLine( g, posy, posx, (int)snap.last[PerfSnap.IDX_DELIVER], (int)prevSnap.last[PerfSnap.IDX_DELIVER], (int)snap.last[PerfSnap.IDX_DELIVERERR], (int)prevSnap.last[PerfSnap.IDX_DELIVERERR], maxheight, colorGraphDeliver);
            }
/*            int polyx[] = new int[viewableGraph];
            int polyDeliver[] = new int[viewableGraph];
            int polyDeliverErr[] = new int[viewableGraph];
            int polyRetry[] = new int[viewableGraph];
            int polySubmit[] = new int[viewableGraph];
            int polySubmitErr[] = new int[viewableGraph];
            ...
            for (int i = 0; i < viewableGraph && idx > 0; i++, cntg++) {
                PerfSnap snap = (PerfSnap) snaps.elementAt(--idx);
                polyx[i] = posx;
                polyDeliver[i] = (int) (posy - (maxheight * snap.last[PerfSnap.IDX_DELIVER]) / scale);
                polyDeliverErr[i] = (int) (posy - (maxheight * snap.last[PerfSnap.IDX_DELIVERERR]) / scale);
                polyRetry[i] = (int) (posy - (maxheight * snap.last[PerfSnap.IDX_RETRY]) / scale);
                polySubmit[i] = (int) (posy - (maxheight * snap.last[PerfSnap.IDX_SUBMIT]) / scale);
                polySubmitErr[i] = (int) (posy - (maxheight * snap.last[PerfSnap.IDX_SUBMITERR]) / scale);
                posx -= pixPerSecond;
            }
            // draw poly lines
            g.setColor(colorGraphSubmitErr);
            g.drawPolyline(polyx, polySubmitErr, cntg);
            g.setColor(colorGraphRetry);
            g.drawPolyline(polyx, polyRetry, cntg);
            g.setColor(colorGraphSubmit);
            g.drawPolyline(polyx, polySubmit, cntg);
            g.setColor(colorGraphDeliverErr);
            g.drawPolyline(polyx, polyDeliverErr, cntg);
            g.setColor(colorGraphDeliver);
            g.drawPolyline(polyx, polyDeliver, cntg); */
/*            {
