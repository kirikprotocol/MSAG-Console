...
<table class=navbar cellspacing=1 cellpadding=0>
    <tr>
        <td class=first><a <%=makeHref("navigatePage(0)")%>><img src="/images/nav_first.gif" width="12" height="11"
                                                                 alt="<%=getLocString("navbar.firstPage")%>"></a></td>
        <%if(pageCurrent > 0){%>
        <td class=prev><a <%=makeHref("navigatePage(" + (pageCurrent - 1) + ")")%>><img src="/images/nav_prev.gif"
                                                                                        width="12" height="11"
                                                                                        alt="<%=getLocString("navbar.prevPage")%>"></a>
        </td>
        <%}
  for(int pageNum=pageFirst;pageNum<=pageLast;pageNum++) {%>
        <td class="<%=pageNum==pageCurrent ? "current":"page"%>"><a <%=makeHref("navigatePage(" + pageNum + ")")%>
                title="<%=getLocString("navbar.pagePre") + pageNum + 1%>"><%=pageNum + 1%></a></td>
        <%}%>
        <%if(pageCurrent+1 < pageTotal){%>
        <td class=next><a <%=makeHref("navigatePage(" + (pageCurrent + 1) + ")")%>><img src="/images/nav_next.gif"
                                                                                        width="12" height="11"
                                                                                        alt="<%=getLocString("navbar.nextPage")%>"></a>
        </td>
        <%}%>
        <td class=last><a <%=makeHref("navigatePage(" + (pageTotal > 0 ? pageTotal - 1 : 0) + ")")%>><img
                src="/images/nav_last.gif" width="12" height="11" alt="<%=getLocString("navbar.lastPage")%>"></a></td>
