...
<%@ page import="ru.novosoft.smsc.jsp.PageBean,
                 ru.novosoft.smsc.jsp.mscman.MscManagerFormBean,
...
<div class=content>
    ...
    <table class=list cellspacing=0>
        ...
        <tr>
            <th>
                <a href="javascript:setSort('commutator')" <%=bean.getSort().endsWith("commutator") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                   title="<%=getLocString("common.sortmodes.commutatorHint")%>"><%=getLocString("common.sortmodes.commutator")%></a>
            </th>
            <th>
                <a href="javascript:setSort('status')" <%=bean.getSort().endsWith("status") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                   title="<%=getLocString("common.sortmodes.statusHint")%>"><%=getLocString("common.sortmodes.status")%></a>
            </th>
            <th>
                <a href="javascript:setSort('failures')" <%=bean.getSort().endsWith("failures") ? (bean.getSort().charAt(0) == '-' ? "class=up" : "class=down") : ""%>
                   title="<%=getLocString("common.sortmodes.failuresHint")%>"><%=getLocString("common.sortmodes.failures")%></a>
            </th>
        ...
        <tr class=row<%=(posIdx++) % 2%>>
            ...
            <td align=right>
                ...
                <%button(out, getLocImageFileName("but_unregister"),
                        "mbUnregister", "UnRegister", "mscman.unregButtonHint", "opForm.mscKey.value='" + commutator + "'; clickSubmit('mbUnregister', 'UnRegister')");%>
            </td>
        </tr><%
        } //for
    %>
