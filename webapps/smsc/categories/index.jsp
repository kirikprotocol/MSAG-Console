...
<table class=list cellspacing=0 cellpadding=0 id=CATEGORY_LIST_TABLE>
    ...
    <tbody>
        ...
        %>
        <tr class=row<%=row & 1%>>
            <td><input class=check type=checkbox name=checkedCategoryNames
                       value="<%=encName%>" <%=bean.isCategoryChecked(name) ? "checked" : ""%>></td>
            <td><%=encCategoryId%>&nbsp;</td>
            <td><a href="javascript:edit('<%=encCategoryId%>','<%=encName%>')"
                   title="<%=getLocString("categories.editTitle")%>"><%=encName%></a></td>
        </tr>
