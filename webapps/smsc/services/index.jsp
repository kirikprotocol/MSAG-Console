...
<script>
    function viewHost(hostId)
    {
        ...
        document.getElementById('jbutton').name = "mbViewHost";
...
<table class=list cellspacing=1 width="100%">
    ...
    <tbody>
        ...
        %>
        <tr class=row<%=row & 1%>>
            <td class=check><input class=check type=checkbox name=serviceIds
                                   value="<%=encodedServiceId%>" <%=serviceIds.contains(serviceId) ? "checked" : ""%>>
            </td>
            <%if (request.isUserInRole("services")) {%><td class=name><a
                href="javascript:editService('<%=encodedServiceId%>')"
                title="<%=getLocString("services.editSubTitle")%>"><%=getLocString("common.links.edit")%></a></td><%}%>
            <td class=name><%
