...
<tr class=row0>
	...
	<td><input class=txtW name=name value="<%=StringEncoderDecoder.encode(bean.getName())%>" <%=isEditing ? "readonly" : ""%>></td>
</tr>
<tr class=row1>
...
<tr class=row0>
	...
	<td><input class=txtW name=maxElements value="<%=bean.getMaxElements()%>" <%=isEditing ? "readonly" : ""%>  validation="positive" onkeyup="resetValidation(this)"></td>
</tr>
</table>
...
for (int i = 0; i<bean.getSubmitters().length; i++)
{
	%><tr class=row0>
		<th class=label>&nbsp;</th>
		<td><input class=txtW name=submitters value="<%=StringEncoderDecoder.encode(bean.getSubmitters()[i])%>" validation="address" onkeyup="resetValidation(this)"></td>
	</tr>
	<%
}
%>
<tr class=row0>
	...
	<td><input class=txtW name=submitters validation="address" onkeyup="resetValidation(this)"></td>
</tr>
</table>
...
for (int i = 0; i<bean.getMembers().length; i++)
{
	%><tr class=row0>
		<th class=label>&nbsp;</th>
		<td><input class=txtW name=members value="<%=StringEncoderDecoder.encode(bean.getMembers()[i])%>" validation="address" onkeyup="resetValidation(this)"></td>
	</tr>
	<%
}
%>
