...
<%@ page import="ru.novosoft.smsc.admin.route.Mask,
                 ru.novosoft.smsc.jsp.dl.dlEdit" %>
    ...
    <table class=properties_list cellspacing=0>
        ...
        <tr class=row0>
            ...
            <td><%
                ...
                if (bean.isCreate()) {
            %><input class=txt name=address value="<%=addressEnc%>" validation="address"
                     onkeyup="resetValidation(this)"><%
            } else {
            %><input type=hidden name=address value="<%=addressEnc%>"><input class=txt name=addressToShow
                                                                             value="<%=addressToShowEnc%>" readonly><%
                }
            %></td>
        </tr>
        <tr class=row1>
            <th><%=getLocString("dl.maximumLists")%></th>
            <td><input class=txt name=max_lst value="<%=bean.getMax_lst()%>" validation="positive"
                       onkeyup="resetValidation(this)"></td>
        </tr>
        <tr class=row0>
            <th><%=getLocString("dl.maximumElements")%></th>
            <td><input class=txt name=max_el value="<%=bean.getMax_el()%>" validation="positive"
                       onkeyup="resetValidation(this)"></td>
        </tr>
    </table>
</div><%
