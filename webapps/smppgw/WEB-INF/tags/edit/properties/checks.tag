...
<sm-ep:property title="${title}"
 ...
 /><c:forEach var="i" items="${values}" varStatus="st"
  ><input class=check type=checkbox name="${name}" id="id_${fn:escapeXml(i)}" value="${i}" <c:if test="${smf:contains(checks, i)}">checked</c:if>>&nbsp;<label for="id_${fn:escapeXml(i)}">${valueTitles[st.count-1]}</label><c:if test="${st.count < fn:length(values)}"><br>
</c:if></c:forEach
></sm-ep:property>